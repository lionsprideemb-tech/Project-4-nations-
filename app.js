...
function newWorld(){
  ...
  world={ schemaVersion:52,id:uid(),name,era,historyMode,createdAt:new Date().toISOString(),lastSavedAt:null,clock:{year:initialYear(era),seasonIndex:0,day:1,hour:7},environment:{weather:'Clear'},characters:[],activeCharacterId:null,npcs:[
...
function renderHowToPlay(){
 if(!$('howToPlayPanel')||!world)return;const ch=activeCharacter();if(!ch)return;
 $('howToPlayPanel').innerHTML=`<div class="section-head"><div><p class="eyebrow">HOW TO PLAY</p><h3>You describe what ${escapeHtml(ch.name)} attempts.</h3></div></div>
 <p>The game is played from the <strong>Freeform Action</strong> box. Type an action in first person—talk to someone, go somewhere, work, train, investigate, bend, eat, rest, or try something unexpected. The simulation decides what is possible and what happens.</p>
 <div class="tutorial-examples"><button data-tutorial-action="I look around and see who is nearby and what they are doing.">Look around</button><button data-tutorial-action="I go find someone I know and talk to them.">Talk to someone</button><button data-tutorial-action="I walk through the neighborhood and see what catches my attention.">Explore nearby</button><button data-tutorial-action="I check what responsibilities or work I have today.">Check responsibilities</button></div>
 <p class="muted">The suggested actions are examples, not a menu of everything you can do. You can type your own action at any time.</p>`;
 $('howToPlayPanel').querySelectorAll('[data-tutorial-action]').forEach(b=>b.onclick=()=>{const input=$('gmActionInput');if(input){input.value=b.dataset.tutorialAction;input.focus();}});
}
