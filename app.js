...
function newWorld(){
  ...
  world={ schemaVersion:40,id:uid(),name,era,historyMode,createdAt:new Date().toISOString(),lastSavedAt:null,clock:{year:initialYear(era),seasonIndex:0,day:1,hour:7},environment:{weather:'Clear'},characters:[],activeCharacterId:null,npcs:[
...
function ensureBetaState(){if(!world)return;world.betaState=world.betaState||{version:'Beta 1',recoveryCount:0,lastValidation:null,lastBackup:null};}
...
function createCharacter(e){e.preventDefault();const c=formData();if(!c.name){alert('Your character needs a name.');showCreatorStep(0);return;}if(c.personality.length<3){alert('Choose at least 3 personality traits.');showCreatorStep(5);return;}const character={id:uid('pc'),createdAt:new Date().toISOString(),status:'active',...c,visualIdentity:{...c.appearance,nation:c.nation,region:c.region,currentAge:c.age,artNotes:'Persistent visual identity profile — future generated art should preserve these traits.'},skills:{general:{},bending:{}},health:{condition:'Healthy',fatigue:0},inventory:starterInventory(c),equipment:{outfit:null,tool:null,weapon:null,travel:null},properties:[],lodging:null,ownershipLog:[],currency:startingCurrency(c.wealth),reputation:[],memories:[],familyIds:[],relationshipIds:[],chronicle:[]};
  const family=generateFamily(character);family.forEach(n=>{world.npcs.push(n);character.familyIds.push(n.id);const rel={id:uid('rel'),a:character.id,b:n.id,type:'family',familyRole:n.role,trust:n.relationship.trust,respect:n.relationship.respect,affection:n.relationship.affection,romanticInterest:0,status:'active'};world.relationships.push(rel);character.relationshipIds.push(rel.id);});const friend=createStartingFriend(character);character.relationshipIds.push(world.relationships[world.relationships.length-1].id);character.memories.push({id:uid('memory'),importance:'major',summary:`Began life in ${character.region} with ${family.length} close family connection${family.length===1?'':'s'} and childhood friend ${friend.name}.`,createdAtWorldDate:worldDate()});world.characters.push(character);world.activeCharacterId=character.id;applyOnboardingSettings(character);ensureCharacterLifeState(character);ensureTrainingState(character);ensureHomeSettlement(character);[...family,friend].filter(Boolean).forEach(n=>{n.settlementId=character.homeSettlementId;n.homeSettlementId=character.homeSettlementId;const st=world.settlements.find(s=>s.id===character.homeSettlementId);if(st){st.localNpcIds=Array.isArray(st.localNpcIds)?st.localNpcIds:[];if(!st.localNpcIds.includes(n.id))st.localNpcIds.push(n.id);}});world.onboardingState=world.onboardingState||{completedCharacterIds:[],firstDayCharacterId:null};world.onboardingState.firstDayCharacterId=character.id;world.events.push(makeEvent('A new life begins',`${character.name}, a ${character.age}-year-old ${bendingLabel(character.bending.element).toLowerCase()} from ${character.region}, begins their playable life.`,[character.id]));saveWorld();resetCreator();openTab('firstDayView');renderFirstDay();}
...
function buildFirstDay(ch){
 ensureCharacterLifeState(ch);const st=currentSettlement(ch),friend=relationshipPeople(ch).find(x=>x.r.type==='friend'||x.r.type==='rival-friend')?.n,family=(ch.familyIds||[]).map(personById).filter(Boolean),place=currentPlace(ch),duties=[];
 ...
 if(friend)duties.push(`${friend.name}, your ${friend.role.toLowerCase()}, already knows you well enough to have expectations.`);
...
function renderFirstDay(){
 if(!$('firstDayView')||!world)return;const ch=activeCharacter();if(!ch)return;const d=buildFirstDay(ch);$('firstDayTitle').textContent=`Your First Morning in ${d.st.name}`;$('firstDayIntro').textContent=d.intro;$('firstDayCharacter').innerHTML=`<div class="review-hero"><div class="avatar-placeholder">${escapeHtml(ch.name.slice(0,1).toUpperCase())}</div><div><h3>${escapeHtml(ch.name)}</h3><p>${escapeHtml(ch.occupation)} · ${escapeHtml(bendingLabel(ch.bending.element))}</p><p>${escapeHtml(d.place?.name||d.st.name)} · ${escapeHtml(world.environment.weather)}</p></div></div>`;
...
function beginFirstDayChoice(kind){
 const ch=activeCharacter();if(!ch)return;world.onboardingState=world.onboardingState||{completedCharacterIds:[],firstDayCharacterId:null};if(!world.onboardingState.completedCharacterIds.includes(ch.id))world.onboardingState.completedCharacterIds.push(ch.id);world.onboardingState.firstDayCharacterId=null;
 if(kind==='work')openTab('careerView');else if(kind==='friend'){const friend=relationshipPeople(ch).find(x=>x.r.type==='friend'||x.r.type==='rival-friend')?.n;if(friend)startDialogue(friend.id);else openTab('dialogueView');}else if(kind==='events')openTab('eventsView');else openTab('lifeView');saveWorld();
}
