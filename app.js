...
const personalities = ['Kind','Patient','Curious','Reserved','Outgoing','Disciplined','Creative','Honest','Ambitious','Stubborn','Compassionate','Competitive','Playful','Practical','Idealistic','Protective','Independent','Traditional','Adventurous','Cautious','Dry-humored','Diplomatic','Intense','Gentle','Skeptical','Loyal','Restless','Methodical','Romantic','Stoic','Optimistic','Pessimistic'];
const strengths = ['Calm under pressure','Excellent memory','Athletic','Empathetic','Natural leader','Observant','Quick learner','Resourceful','Persuasive','Strong work ethic','Spiritually centered','Good with animals','Patient teacher','Excellent listener','Mechanically gifted','Streetwise','Strategic thinker','Good negotiator','Fearlessly loyal','Adaptable','Strong sense of direction','Pain tolerant','Excellent cook','Talented artist'];
...
'Jealous','Possessive','People pleaser','Holds grudges','Terrible liar','Compulsive liar','Fear of failure','Fear of abandonment','Claustrophobic','Afraid of spirits','Needlessly competitive','Arrogant','Insecure','Self-sacrificing to a fault','Emotionally guarded','Gullible','Blunt','Poor with money','Gambler at heart','Procrastinates','Perfectionist','Control freak','Easily embarrassed','Naive about romance','Flirtatious at bad times','Cannot ask for help','Overprotective','Rule obsessed','Authority issues','Vindictive','Hotheaded in a fight','Freezes under pressure','Homesick easily','Restless','Nosy','Chronic worrier','Takes criticism personally','Bad at apologizing','Avoids commitment','Too forgiving','Secretive','Proud of their nation to a fault','Dismissive of tradition','Resistant to change','Competitive sibling streak','Always expects the worst','Acts before listening','Talks too much when nervous','Emotionally impulsive'];
...
const creatorSuggestions={
 gender:['Woman','Man','Nonbinary','Genderfluid','Agender','Questioning','Custom…'],
 pronouns:['she/her','he/him','they/them','she/they','he/they','any pronouns','Custom…'],
 height:["4'10\" / 147 cm","5'0\" / 152 cm","5'2\" / 157 cm","5'4\" / 163 cm","5'6\" / 168 cm","5'8\" / 173 cm","5'10\" / 178 cm","6'0\" / 183 cm","6'2\" / 188 cm","6'4\" / 193 cm","Custom…"],
 skin:['Porcelain','Fair neutral','Fair warm','Light olive','Golden beige','Warm tan','Medium neutral brown','Medium golden brown','Copper brown','Deep brown','Dark brown','Ebony','Custom…'],
 eyes:['Dark brown','Brown','Light brown','Hazel','Amber','Green','Gray','Blue-gray','Blue','Custom…'],
 hairColor:['Black','Blue-black','Dark brown','Chestnut brown','Light brown','Auburn','Copper red','Red','Dark blond','Golden blond','Gray','White','Custom…'],
 hairStyle:['Close cropped','Buzzed','Shaved head','Shaved sides with long top','Short and practical','Short textured curls','Messy medium hair','Shoulder-length loose','Straight long hair','Wavy long hair','Curly long hair','Single long braid','Twin braids','Warrior braid','High ponytail','Low ponytail','Topknot','Half-up topknot','Formal bun','Twin buns','Loose with face-framing strands','Traditional wrapped braid','Braided crown','Custom…'],
 face:['Soft round features','Oval face','Square jaw','Angular face','Heart-shaped face','High cheekbones','Broad features','Narrow features','Heavy brows','Gentle features','Weathered features','Freckled face','Custom…'],
 marks:['None','Small eyebrow scar','Scar across one cheek','Chin scar','Burn scar on one arm','Old training scars','Birthmark on neck','Birthmark on face','Traditional family tattoo','Nation-inspired tattoo','Pierced ears','Multiple ear piercings','Custom…'],
 clothing:['Traditional nation clothing','Practical working clothes','Travel-worn layers','Martial training clothes','Simple rural clothing','Merchant-quality fashionable clothes','Formal family attire','Scholar robes','Artisan workwear','Modern city clothing','Weatherproof travel outfit','Custom…'],
 build:['Slender','Lean','Average','Athletic','Compact','Broad','Sturdy','Muscular','Heavyset','Tall and lanky','Custom…'],
 texture:['Straight','Slightly wavy','Wavy','Loose curls','Curly','Tight curls','Coily','Custom…'],
 goals:['Open my own tea house','Become a respected healer','Master my bending without becoming famous','Travel beyond my home region','Support my family financially','Become a skilled artisan','Teach younger benders','Build a quiet family life','Become a respected community leader','Discover what I actually want from life','Start a successful business','Become a professional fighter','Join public service','Study spirits and old traditions','Custom…'],
 fears:['Disappointing my family','Being abandoned','Losing control of my bending','Deep water','Heights','Spirits','Being trapped in my hometown forever','Failing publicly','Becoming like one of my parents','Losing someone I love','War returning','Never becoming good at anything','Being financially dependent on others','Being forgotten','Custom…'],
 background:['My sibling disappeared and my family stopped talking about it.','My family expects me to inherit the family business.','I owe someone money and they have started asking for it back.','I recently ended a relationship and still see that person around town.','I failed an important training exam and still feel embarrassed.','Someone helped me years ago and I still owe them a favor.','I witnessed something strange involving a spirit and never told anyone.','My closest friend left suddenly and has now returned.','My family has an old reputation I do not know whether I want.','I am saving money for a life my family does not understand.','I promised someone I would accomplish something important for them.','Custom…'],
 storyWish:['Surprise me','A personal coming-of-age story','Family drama and legacy','Romance and relationships','Career and ambition','Mystery tied to my past','Spiritual mystery','Political intrigue','Martial arts and competition','Adventure and travel','Mostly slice-of-life with occasional danger','A balanced mix','Custom…']
};
...
function newWorld(){
  ...
  world={ schemaVersion:54,id:uid(),name,era,historyMode,createdAt:new Date().toISOString(),lastSavedAt:null,clock:{year:initialYear(era),seasonIndex:0,day:1,hour:7},environment:{weather:'Clear'},characters:[],activeCharacterId:null,npcs:[
...
function saveWorld(){ trimPersistentCollections();  if(world){if(Array.isArray(world.events)&&world.events.length>1200)world.events=world.events.slice(-1200);if(Array.isArray(world.storyDirector?.memory)&&world.storyDirector.memory.length>500)world.storyDirector.memory=world.storyDirector.memory.slice(-500);if(Array.isArray(world.storyDirector?.rewards)&&world.storyDirector.rewards.length>400)world.storyDirector.rewards=world.storyDirector.rewards.slice(-400);if(Array.isArray(world.everydayEvents?.events)&&world.everydayEvents.events.length>240)world.everydayEvents.events=world.everydayEvents.events.slice(-240);} if(!world)return;world.lastSavedAt=new Date().toISOString();world.saveMeta=world.saveMeta||{slotId:'slot-1',slotName:'Primary World'};localStorage.setItem(STORAGE_KEY,JSON.stringify(world));localStorage.setItem(slotKey(world.saveMeta.slotId),JSON.stringify(world));upsertSlotMeta();applyUiSettings();render();if($('saveStatus'))$('saveStatus').textContent=`Saved · ${world.saveMeta.slotName}`;}
...
function ensureIntegrationState(){
 if(!world)return;world.integrationState=world.integrationState||{version:'Beta 2',lastDailyTick:null,lastYearTick:null,metrics:{},issues:[],contextStats:[]};
 world.integrationState.metrics=world.integrationState.metrics||{};world.integrationState.issues=Array.isArray(world.integrationState.issues)?world.integrationState.issues:[];world.integrationState.contextStats=Array.isArray(world.integrationState.contextStats)?world.integrationState.contextStats:[];
}
function trimPersistentCollections(){
 if(!world)return;
 const trim=(arr,max)=>Array.isArray(arr)&&arr.length>max?arr.slice(-max):arr;
 world.events=trim(world.events,1000);
 if(world.storyDirector){world.storyDirector.memory=trim(world.storyDirector.memory,350);world.storyDirector.rewards=trim(world.storyDirector.rewards,250);}
 if(world.memoryNarration){world.memoryNarration.sharedMoments=trim(world.memoryNarration.sharedMoments,350);world.memoryNarration.callbacks=trim(world.memoryNarration.callbacks,220);}
 if(world.questless){world.questless.beats=trim(world.questless.beats,300);world.questless.threads=trim(world.questless.threads,120);}
 if(world.socialNetwork){world.socialNetwork.events=trim(world.socialNetwork.events,420);world.socialNetwork.gossip=trim(world.socialNetwork.gossip,260);world.socialNetwork.alliances=trim(world.socialNetwork.alliances,180);world.socialNetwork.feuds=trim(world.socialNetwork.feuds,180);}
 if(world.npcAutonomy){world.npcAutonomy.decisions=trim(world.npcAutonomy.decisions,360);world.npcAutonomy.milestones=trim(world.npcAutonomy.milestones,180);}
 if(world.worldEvents2){world.worldEvents2.events=trim(world.worldEvents2.events,120);world.worldEvents2.history=trim(world.worldEvents2.history,160);}
 if(world.generationalWorld){world.generationalWorld.historicalMemory=trim(world.generationalWorld.historicalMemory,240);world.generationalWorld.inheritances=trim(world.generationalWorld.inheritances,160);world.generationalWorld.eras=trim(world.generationalWorld.eras,40);}
 if(world.regionalEvolution)world.regionalEvolution.history=trim(world.regionalEvolution.history,220);
 if(world.culturalEvolution){world.culturalEvolution.history=trim(world.culturalEvolution.history,220);world.culturalEvolution.trends=trim(world.culturalEvolution.trends,140);}
 if(world.everydayEvents)world.everydayEvents.events=trim(world.everydayEvents.events,180);
}
function integratedDailyWorldTick(){
 if(!world)return;ensureIntegrationState();const stamp=`${world.clock.year}-${world.clock.seasonIndex}-${world.clock.day}`;if(world.integrationState.lastDailyTick===stamp)return;world.integrationState.lastDailyTick=stamp;
 // Cross-system consequences that should happen once per simulated day.
 const activeMajor=(world.worldEvents2?.events||[]).filter(e=>e.status!=='resolved');
 activeMajor.forEach(e=>{
   const st=world.settlements.find(s=>s.id===e.settlementId);if(!st)return;
   const locals=residentsOf(st.id).filter(n=>n.alive!==false);
   if(['disaster','crime','health'].includes(e.type)&&e.severity>=55&&locals.length&&Math.random()<.12){
     const n=locals[Math.floor(Math.random()*locals.length)];ensureNpcLife(n);n.autonomy.stress=clamp(n.autonomy.stress+4);n.autonomy.satisfaction=clamp(n.autonomy.satisfaction-2);
   }
   if(['politics','civic','crime'].includes(e.type)&&world.socialNetwork&&Math.random()<.08){
     const candidates=locals.filter(n=>n.autonomy?.sociability>45);if(candidates.length){const speaker=candidates[Math.floor(Math.random()*candidates.length)];gossipAbout(speaker,null,`${e.title} is affecting daily life in ${st.name}.`,{tone:'neutral',importance:1});}
   }
 });
 // Evolved settlement/culture affect NPC satisfaction.
 world.settlements.forEach(st=>{
   const evo=world.regionalEvolution?.settlements?.[st.id],culture=world.culturalEvolution?.regions?.[st.id],econ=world.economyState?.settlements?.[st.id];
   if(!evo||!culture||!econ)return;
   residentsOf(st.id).forEach(n=>{ensureNpcLife(n);if(econ.prosperity>70)n.autonomy.satisfaction=clamp(n.autonomy.satisfaction+1);if(econ.damage>45)n.autonomy.stress=clamp(n.autonomy.stress+1);if(culture.generationGap>35&&n.age<30)n.autonomy.satisfaction=clamp(n.autonomy.satisfaction+(culture.values.innovation>culture.values.tradition?1:0));});
 });
 trimPersistentCollections();
 world.integrationState.metrics={population:world.npcs.filter(n=>n.alive!==false).length,activeWorldEvents:activeMajor.length,openThreads:(world.questless?.threads||[]).filter(x=>x.status==='open').length,socialEvents:world.socialNetwork?.events?.length||0,historicalMemories:world.generationalWorld?.historicalMemory?.length||0};
}
function integratedYearlyWorldTick(){
 if(!world)return;ensureIntegrationState();if(world.integrationState.lastYearTick===world.clock.year)return;world.integrationState.lastYearTick=world.clock.year;
 updateFamilyReputation();world.settlements.forEach(st=>{const evo=evolutionFor(st),culture=cultureFor(st),econ=settlementEconomy(st.id);if(evo.identity.includes('prosperous'))culture.values.innovation=clamp(culture.values.innovation+1);if(evo.identity.includes('spirit-haunted'))culture.values.spirituality=clamp(culture.values.spirituality+2);if(econ.damage>50)culture.values.community=clamp(culture.values.community+2);});
 trimPersistentCollections();
}
function integrationDiagnostics(){
 ensureIntegrationState();const issues=[];
 if(!activeCharacter()&&world.characters.length)issues.push('No valid active character');
 if((world.npcs||[]).length>900)issues.push('Very high NPC count may reduce browser performance');
 if(JSON.stringify(world).length>5_000_000)issues.push('Save is larger than 5 MB');
 if((world.socialNetwork?.gossip||[]).length>300)issues.push('Gossip history unusually large');
 if((world.worldEvents2?.events||[]).filter(e=>e.status!=='resolved').length>12)issues.push('Many simultaneous world events');
 world.integrationState.issues=issues.slice(0,20);return issues;
}
function compactDirectorPacket(ctx){
 const base=storyDirectorContext(ctx),json=JSON.stringify(base);ensureIntegrationState();world.integrationState.contextStats.push({worldDate:worldDate(),bytes:json.length,target:ctx.targetNpc?.id||null});world.integrationState.contextStats=world.integrationState.contextStats.slice(-60);return base;
}
function renderBeta2(){
 if(!$('beta2View')||!world)return;const ch=activeCharacter();$('noBeta2Life').classList.toggle('hidden',!!ch);$('beta2Game').classList.toggle('hidden',!ch);if(!ch)return;ensureIntegrationState();const issues=integrationDiagnostics(),m=world.integrationState.metrics||{},lat=world.director2?.latency||[],avg=lat.length?Math.round(lat.reduce((a,b)=>a+b.ms,0)/lat.length):0,ctx=world.integrationState.contextStats||[],avgCtx=ctx.length?Math.round(ctx.reduce((a,b)=>a+b.bytes,0)/ctx.length):0;
 $('beta2Summary').innerHTML=`<span>Population <b>${m.population||world.npcs.filter(n=>n.alive!==false).length}</b></span><span>Active world events <b>${m.activeWorldEvents||0}</b></span><span>Open story threads <b>${m.openThreads||0}</b></span><span>Avg AI latency <b>${avg?`${(avg/1000).toFixed(1)}s`:'—'}</b></span><span>Avg context <b>${avgCtx?`${Math.round(avgCtx/1024)} KB`:'—'}</b></span>`;
 $('beta2Health').innerHTML=issues.length?issues.map(x=>`<div class="notice"><strong>Watch:</strong> ${escapeHtml(x)}</div>`):'<div class="notice"><strong>Healthy:</strong> No integration warning is currently active.</div>';
 $('beta2Systems').innerHTML=[['World Director 2.0',!!world.director2],['NPC Autonomy',!!world.npcAutonomy],['Social Network',!!world.socialNetwork],['World Events',!!world.worldEvents2],['Questless Stories',!!world.questless],['Memory Narration',!!world.memoryNarration],['Generational World',!!world.generationalWorld],['Regional Evolution',!!world.regionalEvolution],['Cultural Evolution',!!world.culturalEvolution]].map(([n,on])=>`<div class="balance-row"><strong>${n}</strong><span>${on?'Integrated':'Missing'}</span></div>`).join('');
}
...
function advanceHours(hours){ if(!world)return;
      for(let i=0;i<hours;i++){
        ...
        world.clock.hour++;
        if(world.clock.hour>=24){world.clock.hour=0;world.clock.day++;maybeChangeWeather();maybeGenerateAmbientEvent();processHistoricalSimulation();simulateNpcDay();simulateFactionPolitics();economyDailyTick();economySeasonTick();advanceIgnoredArcs();npcInitiatedRelationshipEvents();canonDailyTick();spreadNpcKnowledge();generateEverydayEvent();expireEverydayEvents();dynamicWorldEventDailyTick();questlessDailyTick();memoryNarrationDailyTick();integratedDailyWorldTick();}
        if(world.clock.day>30){world.clock.day=1;world.clock.seasonIndex++;monthlyBalanceTick();}
        if(world.clock.seasonIndex>3){world.clock.seasonIndex=0;world.clock.year++;processHistoricalSimulation(true);simulateNpcYear();simulateGenerationYear();relationshipYearTick();simulateLongTermYear();generationalWorldYearTick();regionalEvolutionYearTick();culturalEvolutionYearTick();integratedYearlyWorldTick();}
...
function ensureQuestlessStorytelling(){
 if(!world)return;world.questless=world.questless||{version:1,threads:[],beats:[],lastDailyTick:null};world.questless.threads=Array.isArray(world.questless.threads)?world.questless.threads:[];world.questless.beats=Array.isArray(world.questless.beats)?world.questless.beats:[];
}
function storySignalsFromAction(ctx){
 const {ch,town,parsed,targetNpc,rawAction,outcome}=ctx,signals=[];const text=String(rawAction||'').toLowerCase();
 if(targetNpc){const r=relationshipWith(ch,targetNpc);signals.push({kind:'person',key:`person:${targetNpc.id}`,label:targetNpc.name,weight:2+(r.closeness||0)/35,summary:`Your choices keep intersecting with ${targetNpc.name}.`});}
 if(['investigate','talk','fight','steal','bend','explore'].includes(parsed.intent))signals.push({kind:'interest',key:`interest:${parsed.intent}:${town.id}`,label:parsed.intent,weight:2,summary:`You have repeatedly chosen to ${parsed.intent} around ${town.name}.`});
 const major=(world.worldEvents2?.events||[]).find(e=>e.settlementId===town.id&&e.status!=='resolved');if(major&&text.match(/help|ask|investigate|follow|join|oppose|organize|watch|look|fight|protect|stop/))signals.push({kind:'world-event',key:`event:${major.id}`,label:major.title,weight:3,summary:`Your actions are becoming entangled with ${major.title}.`});
 const rumor=(world.rumors||[]).find(r=>r.settlementId===town.id&&!r.investigated&&text.split(/\s+/).some(w=>String(r.text||'').toLowerCase().includes(w)&&w.length>5));if(rumor)signals.push({kind:'rumor',key:`rumor:${rumor.id}`,label:'local rumor',weight:3,summary:'You keep pulling at a local unanswered question.'});
 if(outcome?.tier==='failure'||outcome?.tier==='success-cost')signals.push({kind:'consequence',key:`consequence:${parsed.intent}:${town.id}`,label:'unfinished consequence',weight:1.5,summary:'A consequence from your own choices remains unresolved.'});
 return signals;
}
function questlessThreadFor(ch,signal){
 ensureQuestlessStorytelling();let th=world.questless.threads.find(x=>x.characterId===ch.id&&x.key===signal.key&&x.status==='open');
 if(!th){th={id:uid('thread'),characterId:ch.id,key:signal.key,kind:signal.kind,label:signal.label,status:'open',attention:0,commitment:0,createdAt:worldDate(),lastTouched:worldDate(),summary:signal.summary,beats:[],people:[],settlements:[],playerDefined:true};world.questless.threads.push(th);}
 return th;
}
function recognizeQuestlessStory(ctx,narration){
 if(!ctx?.feasibility?.allowed)return;ensureQuestlessStorytelling();const signals=storySignalsFromAction(ctx),ch=ctx.ch;
 signals.forEach(sig=>{const th=questlessThreadFor(ch,sig);th.attention=Math.min(100,th.attention+Math.round(sig.weight*8));th.commitment=Math.min(100,th.commitment+1);th.lastTouched=worldDate();if(ctx.targetNpc&&!th.people.includes(ctx.targetNpc.id))th.people.push(ctx.targetNpc.id);if(!th.settlements.includes(ctx.town.id))th.settlements.push(ctx.town.id);
 const beat={id:uid('beat'),threadId:th.id,characterId:ch.id,worldDate:worldDate(),action:ctx.rawAction,outcome:ctx.outcome.label,narration:String(narration||'').slice(0,420)};th.beats.push(beat);if(th.beats.length>24)th.beats.shift();world.questless.beats.push(beat);if(world.questless.beats.length>400)world.questless.beats.shift();
 if(th.attention>=45&&!th.recognizedAt){th.recognizedAt=worldDate();world.events.push(makeEvent('A personal story is taking shape',`${ch.name}'s repeated choices around ${th.label} have become a meaningful thread.`,[ch.id,...th.people]));}
 });
}
function questlessDailyTick(){
 if(!world)return;ensureQuestlessStorytelling();const stamp=`${world.clock.year}-${world.clock.seasonIndex}-${world.clock.day}`;if(world.questless.lastDailyTick===stamp)return;world.questless.lastDailyTick=stamp;
 world.questless.threads.filter(x=>x.status==='open').forEach(th=>{const daysSince=th.lastTouched===worldDate()?0:1;if(daysSince&&Math.random()<.18)th.attention=Math.max(0,th.attention-1);const event=th.key.startsWith('event:')?world.worldEvents2?.events?.find(e=>`event:${e.id}`===th.key):null;if(event?.status==='resolved'){th.status='changed';th.resolution=`The wider event resolved ${event.resolution||'without a simple ending'}. Your involvement remains part of its history.`;}if(th.attention<=0&&th.commitment<2){th.status='faded';th.resolution='Your attention moved elsewhere, and this possibility faded into ordinary life.';}});
}
function questlessContext(ch){
 ensureQuestlessStorytelling();return world.questless.threads.filter(x=>x.characterId===ch.id&&x.status==='open'&&x.attention>=20).sort((a,b)=>b.attention-a.attention).slice(0,3).map(x=>({label:x.label,kind:x.kind,attention:x.attention,summary:x.summary,recent:x.beats.slice(-2).map(b=>b.action)}));
}
function renderQuestlessStories(){
 if(!$('questlessView')||!world)return;const ch=activeCharacter();$('noQuestlessLife').classList.toggle('hidden',!!ch);$('questlessGame').classList.toggle('hidden',!ch);if(!ch)return;ensureQuestlessStorytelling();const threads=world.questless.threads.filter(x=>x.characterId===ch.id).sort((a,b)=>b.attention-a.attention);
 const open=threads.filter(x=>x.status==='open'),recognized=open.filter(x=>x.attention>=45);$('questlessSummary').innerHTML=`<span>Story threads <b>${open.length}</b></span><span>Strong themes <b>${recognized.length}</b></span><span>Recorded choices <b>${world.questless.beats.filter(x=>x.characterId===ch.id).length}</b></span>`;
 $('questlessThreads').innerHTML=open.length?open.map(th=>`<article class="thread-card"><div class="section-head"><div><p class="eyebrow">${escapeHtml(th.kind.toUpperCase())}</p><h3>${escapeHtml(th.label)}</h3></div><span class="status-pill">Attention ${th.attention}</span></div><p>${escapeHtml(th.summary)}</p><small>${th.attention>=45?'This has become a meaningful part of your life.':'This is only a possibility because you have shown interest.'}</small><details><summary>Your choices that shaped this</summary>${th.beats.slice().reverse().map(b=>`<p>${escapeHtml(b.worldDate)} — ${escapeHtml(b.action)} <small>(${escapeHtml(b.outcome)})</small></p>`).join('')}</details></article>`).join(''):'<p class="muted">No story thread is being tracked yet. Live normally—the system will notice what you repeatedly choose to care about.</p>';
 const past=threads.filter(x=>x.status!=='open').slice(0,16);$('questlessPast').innerHTML=past.length?past.map(th=>`<div class="event"><strong>${escapeHtml(th.label)} · ${escapeHtml(th.status)}</strong><div>${escapeHtml(th.resolution||'This thread changed as life moved on.')}</div></div>`).join(''):'<p class="muted">No personal threads have faded or changed yet.</p>';
}
...
function createPlayerChild(){
 ...
 world.characters.push(child);ensureGenerationCharacter(child);inheritFamilyLegacy(child,[ch,partner]);const hh=householdFor(ch);child.householdId=hh.id;hh.memberIds.push(child.id);ch.childrenIds.push(child.id);ch.familyIds.push(child.id);
...
function ensureGenerationalWorld(){
 if(!world)return;world.generationalWorld=world.generationalWorld||{version:2,lastYearTick:null,families:{},historicalMemory:[],eras:[],inheritances:[]};world.generationalWorld.families=world.generationalWorld.families||{};world.generationalWorld.historicalMemory=Array.isArray(world.generationalWorld.historicalMemory)?world.generationalWorld.historicalMemory:[];world.generationalWorld.eras=Array.isArray(world.generationalWorld.eras)?world.generationalWorld.eras:[];world.generationalWorld.inheritances=Array.isArray(world.generationalWorld.inheritances)?world.generationalWorld.inheritances:[];
}
function familyKeyFor(p){
 const parents=(p.parentIds||p.familyIds||[]).slice(0,2).sort();return parents.length?`line:${parents.join(':')}`:`founder:${p.id}`;
}
function ensureFamilyLegacy(p){
 ensureGenerationalWorld();const key=p.familyLineKey||familyKeyFor(p);p.familyLineKey=key;let f=world.generationalWorld.families[key];if(!f)f=world.generationalWorld.families[key]={key,name:`${p.name} family line`,founderIds:(p.parentIds||[]).length?(p.parentIds||[]):[p.id],memberIds:[],reputation:{community:0,honor:0,prosperity:0,controversy:0},occupations:{},settlements:{},stories:[],createdYear:world.clock.year};if(!f.memberIds.includes(p.id))f.memberIds.push(p.id);return f;
}
function inheritFamilyLegacy(child,parents){
 const primary=parents.map(ensureFamilyLegacy).sort((a,b)=>b.memberIds.length-a.memberIds.length)[0];if(!primary)return;child.familyLineKey=primary.key;if(!primary.memberIds.includes(child.id))primary.memberIds.push(child.id);
 child.legacy=child.legacy||{};child.legacy.generation=Math.max(...parents.map(p=>p.legacy?.generation||1))+1;child.legacy.familyReputation={...primary.reputation};child.legacy.inheritedStories=primary.stories.slice(-8).map(x=>x.summary);
}
function historicalMemoryFromEvent(ev){
 ensureGenerationalWorld();if(!ev)return;const title=ev.title||ev.kind||'Historical event',detail=ev.summary||ev.detail||ev.description||'';if(!detail)return;const row={id:uid('historymem'),year:world.clock.year,worldDate:worldDate(),title,summary:detail.slice(0,360),settlementId:ev.settlementId||null,participantIds:ev.participantIds||[],importance:ev.importance||2};world.generationalWorld.historicalMemory.push(row);if(world.generationalWorld.historicalMemory.length>350)world.generationalWorld.historicalMemory.shift();return row;
}
function updateFamilyReputation(){
 ensureGenerationalWorld();Object.values(world.generationalWorld.families).forEach(f=>{const members=f.memberIds.map(personById).filter(Boolean);f.occupations={};f.settlements={};members.forEach(p=>{const job=p.occupation||p.role||'Unknown';f.occupations[job]=(f.occupations[job]||0)+1;const sid=p.currentSettlementId||p.settlementId;if(sid)f.settlements[sid]=(f.settlements[sid]||0)+1;});
 const living=members.filter(p=>p.alive!==false),pros=living.reduce((n,p)=>n+Number(p.currency||p.money||0),0);f.reputation.prosperity=clamp(Math.round(pros/Math.max(1,living.length*5)));const notable=members.reduce((n,p)=>n+(p.history?.filter?.(x=>/marriage|career|tournament|alliance|recovered/i.test(x.title||x.kind||''))?.length||0),0);f.reputation.community=clamp(notable*3);});
}
function inheritNpcCareer(child){
 const parents=(child.familyIds||[]).map(personById).filter(Boolean).slice(0,2);if(!parents.length||child.occupation!=='Child')return null;const jobs=parents.map(p=>p.occupation||p.role).filter(x=>x&&x!=='Child');if(jobs.length&&Math.random()<.48)return jobs[Math.floor(Math.random()*jobs.length)];return null;
}
function passDownStoriesToYouth(p){
 if(Number(p.age||0)<8)return;ensureFamilyLegacy(p);const family=world.generationalWorld.families[p.familyLineKey],old=world.generationalWorld.historicalMemory.filter(x=>world.clock.year-x.year>=3&&(x.settlementId===(p.settlementId||p.currentSettlementId)||x.participantIds?.some(id=>(p.familyIds||p.parentIds||[]).includes(id)))).slice(-8);p.inheritedHistory=Array.isArray(p.inheritedHistory)?p.inheritedHistory:[];old.forEach(x=>{if(!p.inheritedHistory.some(y=>y.sourceId===x.id)&&Math.random()<.35)p.inheritedHistory.push({sourceId:x.id,title:x.title,summary:x.summary,learnedYear:world.clock.year,source:'family/community story'});});if(p.inheritedHistory.length>20)p.inheritedHistory=p.inheritedHistory.slice(-20);
}
function transferNpcLegacyOnDeath(n){
 if(n.legacyTransferred)return;n.legacyTransferred=true;const heirs=(n.childrenIds||[]).map(personById).filter(x=>x&&x.alive!==false);if(!heirs.length)return;const share=Math.floor(Number(n.money||0)/heirs.length);heirs.forEach(h=>{h.money=Number(h.money||0)+share;const fam=ensureFamilyLegacy(h);fam.stories.push({year:world.clock.year,summary:`${n.name}, a ${n.occupation||n.role}, died at age ${n.age}.`});});n.money=Math.max(0,Number(n.money||0)-share*heirs.length);world.generationalWorld.inheritances.push({id:uid('inherit'),fromId:n.id,toIds:heirs.map(x=>x.id),amountEach:share,worldDate:worldDate()});
}

function ensureRegionalEvolution(){
 if(!world)return;world.regionalEvolution=world.regionalEvolution||{version:1,lastYearTick:null,settlements:{},history:[]};world.regionalEvolution.settlements=world.regionalEvolution.settlements||{};world.regionalEvolution.history=Array.isArray(world.regionalEvolution.history)?world.regionalEvolution.history:[];
 world.settlements.forEach(st=>{if(!world.regionalEvolution.settlements[st.id])world.regionalEvolution.settlements[st.id]={settlementId:st.id,foundedYear:world.clock.year,populationPeak:residentsOf(st.id).length,development:35,districts:[{id:uid('district'),name:'Old Quarter',type:'mixed',foundedYear:world.clock.year}],industries:[],identity:[],milestones:[],growthStreak:0,declineStreak:0,lastPopulation:residentsOf(st.id).length};});
}
function evolutionFor(st){ensureRegionalEvolution();return world.regionalEvolution.settlements[st.id];}
const districtTypes=[['Market Ward','commerce'],['Crafts Quarter','industry'],['Garden District','residential'],['Canal Ward','transport'],['Scholar Quarter','education'],['Temple Ward','spiritual'],['Workers District','industry'],['Harbor Ward','trade'],['Training Quarter','martial']];
function settlementIdentity(st,econ,ev){
 const tags=[];if(econ.prosperity>=72)tags.push('prosperous');if(econ.infrastructure>=72)tags.push('well-connected');if(econ.damage>=35)tags.push('scarred by disaster');if((ev.industries||[]).length>=2)tags.push('industrial');if((world.spiritState?.disturbances||[]).filter(x=>x.settlementId===st.id).length>=3)tags.push('spirit-haunted');if((world.worldEvents2?.history||[]).filter(x=>x.settlementId===st.id&&x.type==='competition').length>=2)tags.push('martial tradition');if(residentsOf(st.id).length>=24)tags.push('crowded');if(residentsOf(st.id).length<=8)tags.push('quiet');return tags.slice(0,5);
}
function addEvolutionMilestone(st,kind,detail){
 const ev=evolutionFor(st),row={id:uid('evolution'),settlementId:st.id,kind,detail,year:world.clock.year,worldDate:worldDate()};ev.milestones.push(row);if(ev.milestones.length>60)ev.milestones.shift();world.regionalEvolution.history.push(row);if(world.regionalEvolution.history.length>300)world.regionalEvolution.history.shift();historicalMemoryFromEvent({title:kind,detail,settlementId:st.id,importance:2});return row;
}
function evolveSettlementYear(st){
 const ev=evolutionFor(st),econ=settlementEconomy(st.id),pop=residentsOf(st.id).length,delta=pop-ev.lastPopulation;ev.populationPeak=Math.max(ev.populationPeak,pop);ev.growthStreak=delta>0?ev.growthStreak+1:0;ev.declineStreak=delta<0?ev.declineStreak+1:0;
 const score=(econ.prosperity-50)*.22+(econ.infrastructure-50)*.18-(econ.damage*.24)+(delta*2);ev.development=clamp(ev.development+Math.round(score/10)+(Math.random()<.5?1:0));
 if((ev.growthStreak>=2||ev.development>=60)&&ev.districts.length<8&&Math.random()<.35){const options=districtTypes.filter(d=>!ev.districts.some(x=>x.name===d[0]));if(options.length){const d=options[Math.floor(Math.random()*options.length)];ev.districts.push({id:uid('district'),name:d[0],type:d[1],foundedYear:world.clock.year});econ.infrastructure=clamp(econ.infrastructure+3);addEvolutionMilestone(st,'New district',`${st.name} developed the ${d[0]} as population and investment reshaped the settlement.`);}}
 if(econ.prosperity>=65&&Math.random()<.3){const jobs=residentsOf(st.id).map(n=>n.occupation).filter(Boolean),counts={};jobs.forEach(x=>counts[x]=(counts[x]||0)+1);const industry=Object.entries(counts).sort((a,b)=>b[1]-a[1])[0]?.[0];if(industry&&!ev.industries.includes(industry)){ev.industries.push(industry);addEvolutionMilestone(st,'Industry established',`${st.name} became locally known for ${industry.toLowerCase()} work.`);}}
 if(ev.declineStreak>=3&&ev.districts.length>1&&Math.random()<.28){const lost=ev.districts.pop();addEvolutionMilestone(st,'District declined',`${lost.name} lost population and activity after several difficult years.`);}
 if(econ.damage>=45&&Math.random()<.35)addEvolutionMilestone(st,'Lasting disaster scars',`${st.name} still bears visible damage from earlier disasters and incomplete repairs.`);
 if(econ.damage>0&&econ.prosperity>55&&econ.infrastructure>50){const repair=Math.min(econ.damage,3+Math.floor(econ.infrastructure/30));econ.damage=clamp(econ.damage-repair);if(econ.damage<12&&Math.random()<.2)addEvolutionMilestone(st,'Recovery',`${st.name} completed a visible stage of long-term recovery.`);}
 ev.identity=settlementIdentity(st,econ,ev);ev.lastPopulation=pop;
}

function ensureCulturalEvolution(){
 if(!world)return;world.culturalEvolution=world.culturalEvolution||{version:1,lastYearTick:null,regions:{},history:[],trends:[]};world.culturalEvolution.regions=world.culturalEvolution.regions||{};world.culturalEvolution.history=Array.isArray(world.culturalEvolution.history)?world.culturalEvolution.history:[];world.culturalEvolution.trends=Array.isArray(world.culturalEvolution.trends)?world.culturalEvolution.trends:[];
 world.settlements.forEach(st=>{if(!world.culturalEvolution.regions[st.id])world.culturalEvolution.regions[st.id]={settlementId:st.id,generation:1,values:{tradition:60,innovation:35,community:65,individualism:35,spirituality:55,martialPrestige:45,bendingPrestige:60},fashion:[],slang:[],foods:[],entertainment:[],traditions:[],socialNorms:[],bendingAttitude:'respected craft and cultural inheritance',generationGap:10,lastShiftYear:world.clock.year};});
}
function cultureFor(st){ensureCulturalEvolution();return world.culturalEvolution.regions[st.id];}
const cultureSeeds={
 fashion:['layered travel coats','embroidered work jackets','short formal capes','wide utility belts','regional beadwork','practical city boots','revived traditional robes'],
 slang:['bright skies','spirits keep you','stone steady','flame-fast','riverwise','no dust on it','clean strike'],
 foods:['street dumpling stalls','spiced noodle bowls','portable tea cakes','smoked vegetable skewers','festival rice sweets','regional fusion stews'],
 entertainment:['public bending exhibitions','street theater','radio dramas','martial tournaments','dance halls','storytelling houses','pro-bending clubs','community festivals'],
 traditions:['neighborhood lantern night','founders remembrance','season-opening feast','ancestor story evening','youth mastery ceremony','district market festival'],
 norms:['young adults commonly travel before settling','mixed-nation households are increasingly ordinary','craft apprenticeships carry strong prestige','public courtship is increasingly accepted','community service is treated as a mark of adulthood','older formal etiquette is becoming less common']
};
function culturalShift(st,kind,value,detail){
 const c=cultureFor(st);if(Array.isArray(c[kind])&&!c[kind].includes(value))c[kind].push(value);const row={id:uid('culture'),settlementId:st.id,kind,value,detail,year:world.clock.year,worldDate:worldDate()};world.culturalEvolution.history.push(row);world.culturalEvolution.trends.push(row);if(world.culturalEvolution.history.length>350)world.culturalEvolution.history.shift();if(world.culturalEvolution.trends.length>180)world.culturalEvolution.trends.shift();c.lastShiftYear=world.clock.year;return row;
}
function evolveCultureYear(st){
 const c=cultureFor(st),ev=evolutionFor(st),econ=settlementEconomy(st.id),pop=residentsOf(st.id).length,young=residentsOf(st.id).filter(n=>n.age>=14&&n.age<30).length,old=residentsOf(st.id).filter(n=>n.age>=50).length,events=(world.worldEvents2?.history||[]).filter(e=>e.settlementId===st.id).slice(-8);
 c.values.innovation=clamp(c.values.innovation+(econ.tech>55?2:0)+(ev.development>60?1:0)-(Math.random()<.35?1:0));c.values.tradition=clamp(c.values.tradition+(old>young?1:-1));c.values.spirituality=clamp(c.values.spirituality+((world.spiritState?.worldBalance||50)<40?2:0)-(ev.development>75?1:0));c.values.martialPrestige=clamp(c.values.martialPrestige+events.filter(e=>e.type==='competition').length);c.values.community=clamp(c.values.community+(events.some(e=>['disaster','health'].includes(e.type))?2:0));c.generationGap=clamp(Math.abs(c.values.innovation-c.values.tradition)+Math.abs(young-old));
 if(c.values.innovation>58&&Math.random()<.28){const k='fashion',v=cultureSeeds[k][Math.floor(Math.random()*cultureSeeds[k].length)];culturalShift(st,k,v,`${v} became noticeably fashionable in ${st.name}.`);}
 if(young>=3&&Math.random()<.22){const k='slang',v=cultureSeeds[k][Math.floor(Math.random()*cultureSeeds[k].length)];culturalShift(st,k,v,`Younger residents around ${st.name} popularized the expression "${v}."`);}
 if(econ.prosperity>55&&Math.random()<.22){const k='foods',v=cultureSeeds[k][Math.floor(Math.random()*cultureSeeds[k].length)];culturalShift(st,k,v,`${v} became part of everyday local food culture.`);}
 if(pop>=10&&Math.random()<.2){const k='entertainment',v=cultureSeeds[k][Math.floor(Math.random()*cultureSeeds[k].length)];culturalShift(st,k,v,`${v} gained a regular audience in ${st.name}.`);}
 if(c.values.tradition>58&&Math.random()<.16){const k='traditions',v=cultureSeeds[k][Math.floor(Math.random()*cultureSeeds[k].length)];culturalShift(st,k,v,`${v} became an established local tradition.`);}
 if(c.generationGap>28&&Math.random()<.2){const k='norms',v=cultureSeeds[k][Math.floor(Math.random()*cultureSeeds[k].length)];culturalShift(st,k,v,`A generational shift took hold: ${v}.`);}
 if(c.values.innovation>70)c.bendingAttitude='a respected tradition increasingly mixed with modern training, sport, and practical work';else if(c.values.tradition>72)c.bendingAttitude='a deeply traditional inheritance tied to family, discipline, and regional identity';else if(c.values.martialPrestige>68)c.bendingAttitude='a prestigious competitive and martial discipline';else c.bendingAttitude='respected craft and cultural inheritance';
 c.generation=Math.max(1,Math.floor((world.clock.year-(c.originYear||initialYear(world.era)))/20)+1);c.originYear=c.originYear||world.clock.year;
}
function culturalEvolutionYearTick(){if(!world)return;ensureCulturalEvolution();if(world.culturalEvolution.lastYearTick===world.clock.year)return;world.culturalEvolution.lastYearTick=world.clock.year;world.settlements.forEach(evolveCultureYear);}
function culturalContext(st){
 const c=cultureFor(st);return {values:c.values,bendingAttitude:c.bendingAttitude,generationGap:c.generationGap,recentFashion:c.fashion.slice(-3),currentSlang:c.slang.slice(-3),popularFoods:c.foods.slice(-3),entertainment:c.entertainment.slice(-3),traditions:c.traditions.slice(-3),socialNorms:c.socialNorms.slice(-3),recentChanges:world.culturalEvolution.history.filter(x=>x.settlementId===st.id).slice(-4).map(x=>x.detail)};
}
function renderCulturalEvolution(){
 if(!$('culturalEvolutionView')||!world)return;const ch=activeCharacter();$('noCulturalEvolutionLife').classList.toggle('hidden',!!ch);$('culturalEvolutionGame').classList.toggle('hidden',!ch);if(!ch)return;ensureCulturalEvolution();const st=currentSettlement(ch),c=cultureFor(st),hist=world.culturalEvolution.history.filter(x=>x.settlementId===st.id).slice().reverse().slice(0,24);
 $('culturalEvolutionSummary').innerHTML=`<span>Tradition <b>${c.values.tradition}</b></span><span>Innovation <b>${c.values.innovation}</b></span><span>Community <b>${c.values.community}</b></span><span>Spirituality <b>${c.values.spirituality}</b></span><span>Generation gap <b>${c.generationGap}</b></span>`;
 const group=(title,a)=>`<h4>${title}</h4><p>${a.length?escapeHtml(a.slice(-5).join(' · ')):'No distinctive trend has established itself yet.'}</p>`;
 $('currentCultureEvolution').innerHTML=`<p class="eyebrow">${escapeHtml(st.name)} · cultural generation ${c.generation}</p><h3>${escapeHtml(c.bendingAttitude)}</h3>${group('Fashion',c.fashion)}${group('Slang',c.slang)}${group('Food culture',c.foods)}${group('Entertainment',c.entertainment)}${group('Traditions',c.traditions)}${group('Social norms',c.socialNorms)}`;
 $('culturalHistory').innerHTML=hist.length?hist.map(x=>`<div class="event"><strong>${escapeHtml(x.kind)} · Year ${x.year}</strong><div>${escapeHtml(x.detail)}</div></div>`).join(''):'<p class="muted">No major cultural shifts have been recorded here yet.</p>';
}

function regionalEvolutionYearTick(){
 if(!world)return;ensureRegionalEvolution();if(world.regionalEvolution.lastYearTick===world.clock.year)return;world.regionalEvolution.lastYearTick=world.clock.year;world.settlements.forEach(evolveSettlementYear);
}
function settlementEvolutionContext(st){
 const ev=evolutionFor(st),econ=settlementEconomy(st.id);return {development:ev.development,districts:ev.districts.map(x=>x.name),industries:ev.industries.slice(-4),identity:ev.identity,population:residentsOf(st.id).length,populationPeak:ev.populationPeak,prosperity:econ.prosperity,infrastructure:econ.infrastructure,damage:econ.damage,recentChanges:ev.milestones.slice(-4).map(x=>x.detail)};
}
function renderRegionalEvolution(){
 if(!$('regionalEvolutionView')||!world)return;const ch=activeCharacter();$('noRegionalEvolutionLife').classList.toggle('hidden',!!ch);$('regionalEvolutionGame').classList.toggle('hidden',!ch);if(!ch)return;ensureRegionalEvolution();const town=currentSettlement(ch),ev=evolutionFor(town),econ=settlementEconomy(town.id);
 $('regionalEvolutionSummary').innerHTML=`<span>Population <b>${residentsOf(town.id).length}</b></span><span>Development <b>${ev.development}</b></span><span>Prosperity <b>${econ.prosperity}</b></span><span>Infrastructure <b>${econ.infrastructure}</b></span><span>Damage <b>${econ.damage}</b></span>`;
 $('settlementEvolutionPanel').innerHTML=`<p class="eyebrow">${escapeHtml((ev.identity||[]).join(' · ')||'developing settlement')}</p><h3>${escapeHtml(town.name)}</h3><h4>Districts</h4>${ev.districts.map(d=>`<div class="evolution-row"><strong>${escapeHtml(d.name)}</strong><span>${escapeHtml(d.type)} · since Year ${d.foundedYear}</span></div>`).join('')}<h4>Known industries</h4><p>${ev.industries.length?escapeHtml(ev.industries.join(', ')):'No dominant industry has emerged.'}</p>`;
 $('regionalEvolutionHistory').innerHTML=ev.milestones.length?ev.milestones.slice().reverse().map(x=>`<div class="event"><strong>${escapeHtml(x.kind)} · Year ${x.year}</strong><div>${escapeHtml(x.detail)}</div></div>`).join(''):'<p class="muted">This settlement has not accumulated major physical changes yet.</p>';
 const others=world.settlements.filter(x=>x.id!==town.id).map(st=>({st,ev:evolutionFor(st),econ:settlementEconomy(st.id)})).sort((a,b)=>b.ev.development-a.ev.development).slice(0,12);$('regionalComparison').innerHTML=others.map(x=>`<div class="event"><strong>${escapeHtml(x.st.name)}</strong><div>Development ${x.ev.development} · population ${residentsOf(x.st.id).length} · prosperity ${x.econ.prosperity}${x.ev.identity.length?` · ${escapeHtml(x.ev.identity.join(', '))}`:''}</div></div>`).join('');
}

function generationalWorldYearTick(){
 if(!world)return;ensureGenerationalWorld();if(world.generationalWorld.lastYearTick===world.clock.year)return;world.generationalWorld.lastYearTick=world.clock.year;
 [...world.characters,...world.npcs].forEach(p=>{ensureFamilyLegacy(p);passDownStoriesToYouth(p);if(p.alive===false)transferNpcLegacyOnDeath(p);});
 updateFamilyReputation();
 const resolved=(world.worldEvents2?.history||[]).filter(e=>e.resolvedAt&&Number(e.resolvedAt.match(/Year (\d+)/)?.[1])===world.clock.year-1);resolved.forEach(e=>{if(!world.generationalWorld.historicalMemory.some(x=>x.title===e.title&&x.year===world.clock.year-1))historicalMemoryFromEvent({...e,importance:e.severity>60?3:2});});
 if(world.clock.year%10===0)world.generationalWorld.eras.push({id:uid('era'),year:world.clock.year,label:`Year ${world.clock.year} generation`,population:world.npcs.filter(n=>n.alive!==false).length,summary:`A decade marker recording ${Object.keys(world.generationalWorld.families).length} family lines and ${world.generationalWorld.historicalMemory.length} remembered historical events.`});
}
function generationalContext(ch){
 ensureGenerationalWorld();const fam=ensureFamilyLegacy(ch);return {generation:ch.legacy?.generation||1,family:{name:fam.name,reputation:fam.reputation,commonOccupations:Object.entries(fam.occupations).sort((a,b)=>b[1]-a[1]).slice(0,3).map(x=>x[0]),stories:fam.stories.slice(-4)},inheritedHistory:(ch.inheritedHistory||[]).slice(-5),worldHistory:world.generationalWorld.historicalMemory.filter(x=>x.settlementId===ch.currentSettlementId||!x.settlementId).slice(-4).map(x=>({title:x.title,summary:x.summary,year:x.year}))};
}
function renderGenerationalWorld(){
 if(!$('generationalWorldView')||!world)return;const ch=activeCharacter();$('noGenerationalWorldLife').classList.toggle('hidden',!!ch);$('generationalWorldGame').classList.toggle('hidden',!ch);if(!ch)return;ensureGenerationalWorld();const fam=ensureFamilyLegacy(ch),members=fam.memberIds.map(personById).filter(Boolean),hist=world.generationalWorld.historicalMemory.slice().reverse().slice(0,20);
 $('generationalWorldSummary').innerHTML=`<span>Generation <b>${ch.legacy?.generation||1}</b></span><span>Family members <b>${members.length}</b></span><span>Family lines <b>${Object.keys(world.generationalWorld.families).length}</b></span><span>Historical memories <b>${world.generationalWorld.historicalMemory.length}</b></span>`;
 $('familyLegacyPanel').innerHTML=`<h3>${escapeHtml(fam.name)}</h3><div class="rel-metrics"><span>Community ${fam.reputation.community}</span><span>Prosperity ${fam.reputation.prosperity}</span><span>Honor ${fam.reputation.honor}</span><span>Controversy ${fam.reputation.controversy}</span></div>${members.slice(0,14).map(p=>`<div class="event"><strong>${escapeHtml(p.name)}</strong><div>Generation ${p.legacy?.generation||1} · age ${p.age} · ${escapeHtml(p.occupation||p.role||'unestablished')}${p.alive===false?' · deceased':''}</div></div>`).join('')}`;
 $('historicalMemoryPanel').innerHTML=hist.length?hist.map(x=>`<div class="event"><strong>${escapeHtml(x.title)} · Year ${x.year}</strong><div>${escapeHtml(x.summary)}</div></div>`).join(''):'<p class="muted">The world has not yet accumulated long-term historical memory.</p>';
}
...
function ensureMemoryNarration(){
 if(!world)return;world.memoryNarration=world.memoryNarration||{version:1,sharedMoments:[],callbacks:[],lastDailyTick:null};world.memoryNarration.sharedMoments=Array.isArray(world.memoryNarration.sharedMoments)?world.memoryNarration.sharedMoments:[];world.memoryNarration.callbacks=Array.isArray(world.memoryNarration.callbacks)?world.memoryNarration.callbacks:[];
}
function memoryAgeWeight(m){const imp=Number(m.importance||1);return imp>=3?100:imp===2?70:45;}
function relevantNpcMemories(npc,ch,action=''){
 ensureMemoryNarration();ensureNpcLife(npc);const words=new Set(String(action).toLowerCase().split(/\W+/).filter(w=>w.length>4));
 return (npc.memory?.events||[]).filter(m=>!m.forgotten&&(m.aboutId===ch.id||m.importance>=2)).map(m=>{let score=memoryAgeWeight(m);String(m.detail||'').toLowerCase().split(/\W+/).forEach(w=>{if(words.has(w))score+=8;});if(m.aboutId===ch.id)score+=25;if(m.kind==='promise'||m.kind==='secret'||m.kind==='possible lie')score+=30;return {...m,relevance:score};}).sort((a,b)=>b.relevance-a.relevance).slice(0,8);
}
function sharedHistoryContext(ch,npc,action=''){
 const r=relationshipWith(ch,npc),mem=relevantNpcMemories(npc,ch,action),shared=(world.memoryNarration?.sharedMoments||[]).filter(x=>x.characterId===ch.id&&x.npcId===npc.id).slice(-6);
 return {relationship:{type:r.type,trust:r.trust,respect:r.respect,affection:r.affection,tension:r.tension,closeness:r.closeness},importantMemories:mem.map(m=>({kind:m.kind,detail:m.detail,source:m.source,importance:m.importance,worldDate:m.worldDate})),sharedMoments:shared.map(x=>({kind:x.kind,detail:x.detail,worldDate:x.worldDate})),openPromises:(npc.memory?.promises||[]).filter(p=>p.fromId===ch.id&&p.status==='open').slice(-5),suspectedLies:(npc.memory?.liesSuspected||[]).slice(-4)};
}
function recordSharedMoment(ch,npc,kind,detail,importance=2){
 ensureMemoryNarration();const row={id:uid('shared'),characterId:ch.id,npcId:npc.id,kind,detail,importance,worldDate:worldDate()};world.memoryNarration.sharedMoments.push(row);if(world.memoryNarration.sharedMoments.length>500)world.memoryNarration.sharedMoments.shift();npcRemember(npc,kind,detail,{aboutId:ch.id,importance,source:'shared experience'});return row;
}
function detectSharedMoment(ctx,narration){
 if(!ctx.targetNpc||!ctx.feasibility.allowed)return;const kind=ctx.parsed.intent==='fight'?'fight':ctx.parsed.intent==='talk'?'conversation':ctx.parsed.intent==='help'?'favor':'shared event';let importance=['critical-success','failure'].includes(ctx.outcome.tier)?3:1;if(ctx.rawAction.match(/\b(save|protect|betray|promise|love|kiss|break up|forgive|insult|threaten|attack|fight|secret)\b/i))importance=3;if(importance>=2)recordSharedMoment(ctx.ch,ctx.targetNpc,kind,`${ctx.ch.name} and ${ctx.targetNpc.name}: ${ctx.rawAction.slice(0,180)} — ${ctx.outcome.label}.`,importance);
}
function memoryCallbackFor(ctx){
 if(!ctx.targetNpc)return null;const memories=relevantNpcMemories(ctx.targetNpc,ctx.ch,ctx.rawAction);const top=memories[0];if(!top||top.relevance<80)return null;return {npcId:ctx.targetNpc.id,memoryId:top.id,kind:top.kind,detail:top.detail,worldDate:top.worldDate};
}
function recordMemoryCallback(ctx,callback){
 if(!callback)return;ensureMemoryNarration();world.memoryNarration.callbacks.push({id:uid('callback'),characterId:ctx.ch.id,npcId:callback.npcId,memoryId:callback.memoryId,kind:callback.kind,worldDate:worldDate(),detail:`${ctx.targetNpc.name}'s current reaction had a strong reason to recall: ${callback.detail}`});if(world.memoryNarration.callbacks.length>300)world.memoryNarration.callbacks.shift();
}
function memoryNarrationDailyTick(){
 if(!world)return;ensureMemoryNarration();const stamp=`${world.clock.year}-${world.clock.seasonIndex}-${world.clock.day}`;if(world.memoryNarration.lastDailyTick===stamp)return;world.memoryNarration.lastDailyTick=stamp;
 world.npcs.filter(n=>n.alive!==false).forEach(n=>{(n.memory?.events||[]).forEach(m=>{if(m.forgotten||m.importance>=3)return;if(Math.random()<(m.importance===2?.001:.004))m.forgotten=true;});});
}
function renderMemoryNarration(){
 if(!$('memoryNarrationView')||!world)return;const ch=activeCharacter();$('noMemoryNarrationLife').classList.toggle('hidden',!!ch);$('memoryNarrationGame').classList.toggle('hidden',!ch);if(!ch)return;ensureMemoryNarration();const locals=residentsOf(currentSettlement(ch).id),rows=locals.map(n=>({n,mem:relevantNpcMemories(n,ch,'')})).filter(x=>x.mem.length).sort((a,b)=>(b.mem[0]?.relevance||0)-(a.mem[0]?.relevance||0)).slice(0,16);
 $('memoryNarrationSummary').innerHTML=`<span>NPCs remembering you <b>${rows.length}</b></span><span>Shared moments <b>${world.memoryNarration.sharedMoments.filter(x=>x.characterId===ch.id).length}</b></span><span>Narrative callbacks <b>${world.memoryNarration.callbacks.filter(x=>x.characterId===ch.id).length}</b></span>`;
 $('memoryNarrationPeople').innerHTML=rows.length?rows.map(({n,mem})=>`<article class="memory-card"><h3>${escapeHtml(n.name)}</h3><p>${escapeHtml(mem[0].detail)}</p><small>${escapeHtml(mem[0].kind)} · ${escapeHtml(mem[0].source)} · ${escapeHtml(mem[0].worldDate)}</small></article>`).join(''):'<p class="muted">Nobody nearby has a strong memory involving you yet.</p>';
 const cb=world.memoryNarration.callbacks.filter(x=>x.characterId===ch.id).slice().reverse().slice(0,20);$('memoryNarrationCallbacks').innerHTML=cb.length?cb.map(x=>`<div class="event"><strong>${escapeHtml(x.kind)}</strong><div>${escapeHtml(x.detail)}</div><small>${escapeHtml(x.worldDate)}</small></div>`).join(''):'<p class="muted">No strong memory callbacks have been used yet.</p>';
}
...
async function connectedNpcDialogue(session,npc,ch,text){
 const endpoint=world.gmSettings?.endpoint||'/api/gm',timeoutMs=Math.max(5000,Math.min(30000,Number(world.gmSettings?.timeoutMs||18000))),started=performance.now();
...
const worldEventTemplates=[
 {id:'labor-dispute',type:'civic',title:'Labor Dispute',baseChance:.7,duration:[3,8],detail:'Workers and employers are clashing over pay, safety, hours, or working conditions.',pressure:['economy','social']},
 {id:'public-protest',type:'politics',title:'Public Protest',baseChance:.55,duration:[2,6],detail:'Residents are gathering publicly over a local law, decision, shortage, or grievance.',pressure:['politics','social']},
 {id:'local-election',type:'politics',title:'Local Election or Appointment',baseChance:.28,duration:[5,12],detail:'Local political leadership is being contested or reconsidered.',pressure:['politics']},
 {id:'crime-wave',type:'crime',title:'Crime Wave',baseChance:.46,duration:[4,10],detail:'Thefts, intimidation, smuggling, or street violence are becoming more common.',pressure:['crime','economy']},
 {id:'organized-crime',type:'crime',title:'Organized Criminal Pressure',baseChance:.22,duration:[7,16],detail:'A coordinated criminal group is exerting influence over local businesses or residents.',pressure:['crime','politics']},
 {id:'spirit-disturbance',type:'spirit',title:'Spirit Disturbance',baseChance:.34,duration:[3,12],detail:'Unusual spiritual activity is disrupting ordinary routines and local beliefs.',pressure:['spirit']},
 {id:'severe-storm',type:'disaster',title:'Severe Storm',baseChance:.34,duration:[1,4],detail:'A severe storm is damaging roads, homes, crops, or shipping.',pressure:['disaster','economy']},
 {id:'fire-disaster',type:'disaster',title:'Urban or Wildland Fire',baseChance:.16,duration:[1,5],detail:'A destructive fire is threatening property and forcing residents to react quickly.',pressure:['disaster']},
 {id:'flooding',type:'disaster',title:'Flooding',baseChance:.18,duration:[2,6],detail:'Water has overwhelmed streets, farmland, or transport routes.',pressure:['disaster','economy']},
 {id:'tournament',type:'competition',title:'Regional Bending & Martial Tournament',baseChance:.3,duration:[4,9],detail:'Competitors and spectators are gathering for a regional martial or bending tournament.',pressure:['culture']},
 {id:'construction',type:'infrastructure',title:'Major Construction Project',baseChance:.42,duration:[8,22],detail:'A road, bridge, canal, housing block, public hall, port, or other major project is underway.',pressure:['economy','infrastructure']},
 {id:'trade-shortage',type:'economy',title:'Regional Shortage',baseChance:.5,duration:[4,11],detail:'A shortage of important goods is driving prices and public frustration upward.',pressure:['economy']},
 {id:'migration-wave',type:'migration',title:'Migration Wave',baseChance:.25,duration:[5,14],detail:'Families and workers are arriving or leaving in unusually large numbers.',pressure:['migration','economy']},
 {id:'public-health',type:'health',title:'Public Health Strain',baseChance:.2,duration:[4,10],detail:'Illness or sanitation problems are straining households, healers, and workplaces.',pressure:['health','economy']}
];
function ensureWorldEvents2(){
 if(!world)return;world.worldEvents2=world.worldEvents2||{version:2,lastDailyTick:null,events:[],history:[],regionalPressure:{}};
 world.worldEvents2.events=Array.isArray(world.worldEvents2.events)?world.worldEvents2.events:[];world.worldEvents2.history=Array.isArray(world.worldEvents2.history)?world.worldEvents2.history:[];world.worldEvents2.regionalPressure=world.worldEvents2.regionalPressure||{};
}
function eventDayNumber(){return world.clock.year*120+world.clock.seasonIndex*30+world.clock.day;}
function settlementPressures(st){
 ensureWorldEvents2();ensureEconomyState();ensureFactionState();ensureSpiritState();ensureSocialNetwork();
 const econ=settlementEconomy(st.id),locals=residentsOf(st.id),feuds=world.socialNetwork.feuds.filter(x=>x.settlementId===st.id&&x.status==='active').length,gossip=world.socialNetwork.gossip.filter(x=>x.settlementId===st.id).slice(-20).length,incidents=world.factionState.incidents.filter(x=>x.settlementId===st.id).slice(-15).length,spirits=world.spiritState.disturbances.filter(x=>x.settlementId===st.id&&!x.resolved).length;
 const lowStocks=economyGoods.filter(g=>(econ.stocks[g.id]||0)<18).length;
 return {
   economy:clamp((100-econ.prosperity)*.55+lowStocks*10+econ.damage*.45),
   politics:clamp((100-(world.factionState.factions.filter(f=>f.settlementId===st.id).reduce((n,f)=>n+f.stability,0)/Math.max(1,world.factionState.factions.filter(f=>f.settlementId===st.id).length)))*.7+incidents*4),
   crime:clamp(incidents*8+(100-econ.prosperity)*.25+feuds*4),
   social:clamp(feuds*12+gossip*1.5+(locals.length>18?8:0)),
   spirit:clamp(spirits*22+(50-world.spiritState.worldBalance)*.7),
   disaster:clamp((world.environment.weather==='Storm'?45:0)+econ.damage*.5),
   infrastructure:clamp((100-econ.infrastructure)*.8+econ.damage*.4),
   migration:clamp(Math.abs(50-econ.prosperity)*.45+(econ.housing<35?25:0)),
   health:clamp((100-(econ.housing+econ.infrastructure)/2)*.45),
   culture:clamp(30+locals.length)
 };
}
function activeLargeEvents(stId){ensureWorldEvents2();return world.worldEvents2.events.filter(e=>e.settlementId===stId&&['forming','active','resolving'].includes(e.status));}
function eventTemplateChance(t,p){
 let pressure=Math.max(...t.pressure.map(k=>p[k]||0));return Math.min(.18,(t.baseChance/100)+(pressure/1000));
}
function createDynamicWorldEvent(st,t){
 const p=settlementPressures(st),duration=t.duration[0]+Math.floor(Math.random()*(t.duration[1]-t.duration[0]+1)),local=residentsOf(st.id).sort(()=>Math.random()-.5),participantIds=local.slice(0,Math.min(8,2+Math.floor(Math.random()*5))).map(n=>n.id);
 const factions=world.factionState.factions.filter(f=>f.settlementId===st.id).sort(()=>Math.random()-.5).slice(0,2).map(f=>f.id);
 const e={id:uid('worldevent'),templateId:t.id,type:t.type,title:t.title,settlementId:st.id,status:'forming',createdDay:eventDayNumber(),createdAt:worldDate(),durationDays:duration,progress:0,severity:30+Math.floor(Math.random()*40),momentum:45+Math.floor(Math.random()*20),detail:t.detail,participantIds,factionIds:factions,playerInvolvement:0,playerChoice:null,resolution:null,consequences:[],history:[{worldDate:worldDate(),detail:`${t.title} began developing in ${st.name}.`}]};
 world.worldEvents2.events.push(e);if(world.worldEvents2.events.length>160)world.worldEvents2.events.shift();world.events.push(makeEvent(t.title,`${t.detail} The situation is developing in ${st.name}.`,participantIds));return e;
}
function maybeSpawnDynamicWorldEvent(){
 ensureWorldEvents2();world.settlements.forEach(st=>{
   if(activeLargeEvents(st.id).length>=2)return;const p=settlementPressures(st),templates=worldEventTemplates.filter(t=>!activeLargeEvents(st.id).some(e=>e.templateId===t.id));
   const candidates=templates.filter(t=>Math.random()<eventTemplateChance(t,p));if(!candidates.length)return;
   const chosen=candidates.sort((a,b)=>Math.max(...b.pressure.map(k=>p[k]||0))-Math.max(...a.pressure.map(k=>p[k]||0)))[0];createDynamicWorldEvent(st,chosen);
 });
}
function worldEventActorStrength(e){
 const people=(e.participantIds||[]).map(id=>world.npcs.find(n=>n.id===id)).filter(Boolean),factions=(e.factionIds||[]).map(id=>world.factionState.factions.find(f=>f.id===id)).filter(Boolean);
 const social=people.reduce((n,p)=>n+(p.autonomy?.ambition||40)+(p.autonomy?.sociability||40),0)/Math.max(1,people.length)/2;
 const power=factions.reduce((n,f)=>n+f.power+f.stability,0)/Math.max(1,factions.length)/2;return Math.round((social+power)/2);
}
function applyWorldEventConsequence(e,outcome){
 const st=world.settlements.find(s=>s.id===e.settlementId),econ=settlementEconomy(e.settlementId),severity=e.severity;
 const push=(detail)=>{e.consequences.push(detail);e.history.push({worldDate:worldDate(),detail});};
 if(e.type==='economy'){if(outcome==='bad'){econ.prosperity=clamp(econ.prosperity-5);economyGoods.forEach(g=>econ.stocks[g.id]=Math.max(0,econ.stocks[g.id]-3));push('Prices and shortages worsened after the event.');}else{econ.prosperity=clamp(econ.prosperity+3);push('Trade conditions stabilized and local confidence improved.');}}
 if(e.type==='infrastructure'){if(outcome==='good'){econ.infrastructure=clamp(econ.infrastructure+7);econ.prosperity=clamp(econ.prosperity+2);push('The completed project permanently improved local infrastructure.');}else{econ.damage=clamp(econ.damage+4);push('The project stalled or failed, leaving wasted resources and frustration.');}}
 if(e.type==='disaster'){econ.damage=clamp(econ.damage+(outcome==='bad'?12:5));econ.prosperity=clamp(econ.prosperity-(outcome==='bad'?6:2));push(outcome==='bad'?'The disaster caused significant long-term damage.':'Damage was contained, though repairs will still be needed.');}
 if(e.type==='crime'){const law=world.factionState.factions.find(f=>f.settlementId===e.settlementId&&f.type==='law');if(law)law.stability=clamp(law.stability+(outcome==='good'?5:-6));push(outcome==='good'?'Local authorities and residents reduced criminal pressure.':'Criminal influence became more entrenched.');}
 if(e.type==='politics'||e.type==='civic'){const gov=world.factionState.factions.find(f=>f.settlementId===e.settlementId&&(f.type==='government'||f.type==='law'));if(gov)gov.stability=clamp(gov.stability+(outcome==='good'?3:-4));push(outcome==='good'?'The dispute reached a workable settlement.':'The dispute left deeper political resentment.');}
 if(e.type==='spirit'){world.spiritState.worldBalance=clamp(world.spiritState.worldBalance+(outcome==='good'?5:-5));push(outcome==='good'?'Spiritual tension eased.':'The disturbance left the local spiritual balance worse than before.');}
 if(e.type==='migration'){const moveCount=outcome==='bad'?Math.min(5,Math.ceil(severity/15)):Math.min(3,Math.ceil(severity/25));const movers=residentsOf(e.settlementId).sort(()=>Math.random()-.5).slice(0,moveCount);movers.forEach(n=>{const dest=world.settlements.filter(s=>s.id!==st.id).sort(()=>Math.random()-.5)[0];if(dest){n.settlementId=dest.id;n.lifeStatus='migrant';}});push(`${movers.length} resident${movers.length===1?'':'s'} relocated as part of the migration pressure.`);}
 if(e.type==='health'){residentsOf(e.settlementId).sort(()=>Math.random()-.5).slice(0,outcome==='bad'?4:2).forEach(n=>{n.health.condition=outcome==='bad'?'ill':'recovering';n.health.vitality=clamp(n.health.vitality-(outcome==='bad'?10:4));});push(outcome==='bad'?'Illness spread more widely through the community.':'The health strain was brought largely under control.');}
 if(e.type==='competition'&&outcome==='good'){const winner=(e.participantIds||[]).map(id=>world.npcs.find(n=>n.id===id)).filter(Boolean).sort((a,b)=>(b.autonomy?.skills?.combat||0)-(a.autonomy?.skills?.combat||0))[0];if(winner){winner.autonomy.skills.combat=clamp(winner.autonomy.skills.combat+5);push(`${winner.name} emerged as a notable tournament performer.`);}}
}
function resolveDynamicWorldEvent(e){
 if(e.status==='resolved')return;const actors=worldEventActorStrength(e),pressure=settlementPressures(world.settlements.find(s=>s.id===e.settlementId)),relevant=Math.max(...(worldEventTemplates.find(t=>t.id===e.templateId)?.pressure||[]).map(k=>pressure[k]||0),0),roll=actors+e.momentum+e.playerInvolvement*8+Math.floor(Math.random()*40)-relevant;
 const outcome=roll>=80?'good':roll>=48?'mixed':'bad';e.status='resolved';e.resolution=outcome;e.resolvedAt=worldDate();e.progress=100;applyWorldEventConsequence(e,outcome);
 const st=world.settlements.find(s=>s.id===e.settlementId),summary=`${e.title} in ${st?.name||'the region'} resolved ${outcome==='good'?'favorably':outcome==='mixed'?'with mixed results':'poorly'}${e.playerInvolvement?` after player involvement`:''}.`;
 e.history.push({worldDate:worldDate(),detail:summary});world.worldEvents2.history.push({...e,summary});if(world.worldEvents2.history.length>220)world.worldEvents2.history.shift();world.events.push(makeEvent('World event resolved',summary,e.participantIds||[]));
}
function progressDynamicWorldEvents(){
 ensureWorldEvents2();const today=eventDayNumber();world.worldEvents2.events.filter(e=>e.status!=='resolved').forEach(e=>{
   const age=today-e.createdDay;if(e.status==='forming'&&age>=1)e.status='active';if(e.status==='active'){e.progress=Math.min(90,Math.round((age/Math.max(1,e.durationDays))*100));e.momentum=clamp(e.momentum+(Math.random()<.5?2:-2));}
   if(age>=e.durationDays)e.status='resolving';if(e.status==='resolving')resolveDynamicWorldEvent(e);
 });
}
function dynamicWorldEventDailyTick(){
 if(!world)return;ensureWorldEvents2();const stamp=`${world.clock.year}-${world.clock.seasonIndex}-${world.clock.day}`;if(world.worldEvents2.lastDailyTick===stamp)return;world.worldEvents2.lastDailyTick=stamp;progressDynamicWorldEvents();if(Math.random()<.28)maybeSpawnDynamicWorldEvent();
}
function influenceDynamicWorldEvent(id,choice){
 const ch=activeCharacter(),e=world.worldEvents2.events.find(x=>x.id===id&&x.status!=='resolved');if(!ch||!e)return;
 e.playerChoice=choice;e.playerInvolvement=(e.playerInvolvement||0)+1;let delta=0,detail='';
 if(choice==='help'){delta=8;detail=`${ch.name} actively helped people dealing with ${e.title}.`;applyNeeds(ch,{energy:-12,hunger:5,morale:4});}
 if(choice==='organize'){delta=12;detail=`${ch.name} tried to organize a coordinated response to ${e.title}.`;applyNeeds(ch,{energy:-16,hunger:7,morale:3});}
 if(choice==='oppose'){delta=-5;detail=`${ch.name} actively opposed one side or response to ${e.title}.`;applyNeeds(ch,{energy:-10,morale:-1});}
 if(choice==='observe'){delta=2;detail=`${ch.name} observed ${e.title} closely without taking a leading role.`;applyNeeds(ch,{energy:-4});}
 e.momentum=clamp(e.momentum+delta);e.history.push({worldDate:worldDate(),detail});e.participantIds=[...new Set([...(e.participantIds||[]),ch.id])];addLifeLog(ch,e.title,detail);advanceHours(choice==='organize'?4:choice==='help'?3:1);saveWorld();
}
function renderDynamicWorldEvents(){
 if(!$('worldEventsView')||!world)return;const ch=activeCharacter();$('noWorldEventsLife').classList.toggle('hidden',!!ch);$('worldEventsGame').classList.toggle('hidden',!ch);if(!ch)return;ensureWorldEvents2();const town=currentSettlement(ch),active=world.worldEvents2.events.filter(e=>e.settlementId===town.id&&e.status!=='resolved').slice().reverse(),history=world.worldEvents2.history.filter(e=>e.settlementId===town.id).slice().reverse().slice(0,16),p=settlementPressures(town);
 $('worldEventsSummary').innerHTML=`<span>Active <b>${active.length}</b></span><span>Economic pressure <b>${p.economy}</b></span><span>Political pressure <b>${p.politics}</b></span><span>Crime pressure <b>${p.crime}</b></span><span>Spirit pressure <b>${p.spirit}</b></span>`;
 $('activeWorldEvents').innerHTML=active.length?active.map(e=>`<article class="world-event-card"><div class="section-head"><div><p class="eyebrow">${escapeHtml(e.type.toUpperCase())} · ${escapeHtml(e.status.toUpperCase())}</p><h3>${escapeHtml(e.title)}</h3></div><span class="status-pill">${e.progress}%</span></div><p>${escapeHtml(e.detail)}</p><div class="route-facts"><span>Severity ${e.severity}</span><span>Momentum ${e.momentum}</span><span>${e.durationDays} day window</span></div><div class="actions compact"><button data-world-event="${e.id}" data-world-choice="observe">Observe</button><button data-world-event="${e.id}" data-world-choice="help">Help</button><button data-world-event="${e.id}" data-world-choice="organize">Organize</button><button data-world-event="${e.id}" data-world-choice="oppose">Oppose</button></div></article>`).join(''):'<p class="muted">No major regional event is currently underway here.</p>';
 $('activeWorldEvents').querySelectorAll('[data-world-event]').forEach(b=>b.onclick=()=>influenceDynamicWorldEvent(b.dataset.worldEvent,b.dataset.worldChoice));
 $('worldEventHistory').innerHTML=history.length?history.map(e=>`<div class="event"><strong>${escapeHtml(e.title)} · ${escapeHtml(e.resolution||'resolved')}</strong><div>${escapeHtml(e.summary||e.consequences?.slice(-1)?.[0]||'The event ended.')}</div><small>${escapeHtml(e.resolvedAt||e.createdAt)}</small></div>`).join(''):'<p class="muted">No major local world events have resolved yet.</p>';
}
...
function ensureNpcSimulation(){
 if(!world)return;world.npcSimulation=world.npcSimulation||{lastDailyTick:null,lastYearTick:null,households:[],socialEvents:[]};world.npcAutonomy=world.npcAutonomy||{version:2,lastDailyTick:null,decisions:[],milestones:[]};
...
function ensureNpcLife(n,i=0){
 ...
 n.autonomy=n.autonomy||{ambition:35+Math.floor(Math.random()*45),sociability:25+Math.floor(Math.random()*60),risk:15+Math.floor(Math.random()*55),stability:25+Math.floor(Math.random()*60),familyDrive:20+Math.floor(Math.random()*65),wanderlust:15+Math.floor(Math.random()*65),trainingDrive:15+Math.floor(Math.random()*60),stress:10+Math.floor(Math.random()*30),satisfaction:45+Math.floor(Math.random()*30),lastDecision:null,decisionCooldown:0,opinions:{},skills:{work:10+Math.floor(Math.random()*30),combat:5+Math.floor(Math.random()*25),social:10+Math.floor(Math.random()*35)},maritalStatus:n.partnerId?'partnered':'single'};
 n.autonomy.opinions=n.autonomy.opinions||{};n.autonomy.skills=n.autonomy.skills||{work:15,combat:10,social:15};
}
...
function npcRelation(a,b){ensureNpcLife(a);ensureNpcLife(b);a.npcRelations[b.id]=a.npcRelations[b.id]||{trust:10+Math.floor(Math.random()*20),affection:5+Math.floor(Math.random()*20),respect:8+Math.floor(Math.random()*18),tension:0,closeness:8,jealousy:0,loyalty:12,type:'acquaintance',status:'active',history:[],sharedGroups:[]};const r=a.npcRelations[b.id];r.respect=r.respect??10;r.jealousy=r.jealousy??0;r.loyalty=r.loyalty??12;r.type=r.type||'acquaintance';r.status=r.status||'active';r.history=Array.isArray(r.history)?r.history:[];r.sharedGroups=Array.isArray(r.sharedGroups)?r.sharedGroups:[];return r;}
...
function ensureSocialNetwork(){
 if(!world)return;world.socialNetwork=world.socialNetwork||{version:2,lastDailyTick:null,events:[],gossip:[],alliances:[],feuds:[]};
 world.socialNetwork.events=Array.isArray(world.socialNetwork.events)?world.socialNetwork.events:[];world.socialNetwork.gossip=Array.isArray(world.socialNetwork.gossip)?world.socialNetwork.gossip:[];world.socialNetwork.alliances=Array.isArray(world.socialNetwork.alliances)?world.socialNetwork.alliances:[];world.socialNetwork.feuds=Array.isArray(world.socialNetwork.feuds)?world.socialNetwork.feuds:[];
}
function npcRelationType(r){
 if(r.tension>=70&&r.trust<25)return 'enemy';if(r.tension>=45&&r.respect>35)return 'rival';if(r.trust>=72&&r.affection>=65&&r.closeness>=65)return 'close friend';if(r.trust>=52&&r.affection>=42)return 'friend';if(r.trust>=28||r.closeness>=28)return 'familiar';return 'acquaintance';
}
function syncNpcRelationType(a,b){
 const r=npcRelation(a,b),back=npcRelation(b,a),old=r.type;r.type=npcRelationType(r);back.type=npcRelationType(back);
 if(old!==r.type&&['close friend','enemy','rival'].includes(r.type))recordSocialEvent(a,b,'Relationship changed',`${a.name} now sees ${b.name} as ${r.type}.`,2);
}
function socialEventRow(a,b,kind,detail,importance=1,extra={}){
 return {id:uid('social'),aId:a?.id||null,bId:b?.id||null,kind,detail,importance,settlementId:a?.settlementId||b?.settlementId||null,worldDate:worldDate(),year:world.clock.year,...extra};
}
function recordSocialEvent(a,b,kind,detail,importance=1,extra={}){
 ensureSocialNetwork();const ev=socialEventRow(a,b,kind,detail,importance,extra);world.socialNetwork.events.push(ev);if(world.socialNetwork.events.length>600)world.socialNetwork.events.shift();
 if(a){ensureNpcLife(a);a.history.push({title:kind,detail,worldDate:worldDate(),year:world.clock.year});if(a.history.length>80)a.history.shift();}
 if(b){ensureNpcLife(b);b.history.push({title:kind,detail,worldDate:worldDate(),year:world.clock.year});if(b.history.length>80)b.history.shift();}
 return ev;
}
function sharedSocialNeighbors(a,b){
 const aa=new Set(Object.keys(a.npcRelations||{})),bb=new Set(Object.keys(b.npcRelations||{}));return [...aa].filter(id=>bb.has(id)).map(id=>world.npcs.find(n=>n.id===id)).filter(n=>n&&n.alive!==false);
}
function socialGroupTags(n){
 const tags=[];if(n.occupation)tags.push(`work:${n.occupation}`);if(n.familyIds?.length)tags.push('family');if(n.settlementId)tags.push(`town:${n.settlementId}`);if(n.partnerId)tags.push(`couple:${[n.id,n.partnerId].sort().join(':')}`);return tags;
}
function updateSharedGroups(a,b){
 const r=npcRelation(a,b),tagsA=socialGroupTags(a),tagsB=new Set(socialGroupTags(b));r.sharedGroups=tagsA.filter(x=>tagsB.has(x));return r.sharedGroups;
}
function gossipAbout(source,subject,detail,{tone='neutral',importance=1}={}){
 ensureSocialNetwork();const candidates=residentsOf(source.settlementId).filter(n=>n.id!==source.id&&n.id!==subject?.id&&n.alive!==false);
 const listeners=candidates.sort(()=>Math.random()-.5).slice(0,Math.min(3,1+Math.floor(source.autonomy.sociability/35)));
 listeners.forEach(listener=>{
   ensureNpcLife(listener);const sourceRel=npcRelation(listener,source);if(sourceRel.trust<20&&Math.random()<.65)return;
   const row={id:uid('gossip'),sourceId:source.id,listenerId:listener.id,subjectId:subject?.id||null,detail,tone,importance,settlementId:source.settlementId,worldDate:worldDate()};world.socialNetwork.gossip.push(row);if(world.socialNetwork.gossip.length>400)world.socialNetwork.gossip.shift();
   npcRemember(listener,'gossip',`${source.name} said: ${detail}`,{aboutId:subject?.id||null,importance,source:'heard from another NPC'});
   if(subject){const op=npcOpinion(listener,subject);if(tone==='negative'){op.liking=clamp(op.liking-2,-100,100);op.suspicion=clamp(op.suspicion+3);}if(tone==='positive'){op.liking=clamp(op.liking+2,-100,100);op.respect=clamp(op.respect+2,-100,100);}}
 });
}
function takeSidesInConflict(a,b,detail){
 ensureSocialNetwork();const neighbors=[...new Set([...sharedSocialNeighbors(a,b),...residentsOf(a.settlementId).filter(n=>n.id!==a.id&&n.id!==b.id).slice(0,5)])];
 neighbors.forEach(n=>{const ra=npcRelation(n,a),rb=npcRelation(n,b),scoreA=ra.trust+ra.affection+ra.loyalty-ra.tension,scoreB=rb.trust+rb.affection+rb.loyalty-rb.tension;if(Math.abs(scoreA-scoreB)<18)return;const favored=scoreA>scoreB?a:b,opposed=favored===a?b:a;updateNpcOpinion(n,favored,{respect:2,liking:1});updateNpcOpinion(n,opposed,{suspicion:2,liking:-1});npcRelation(n,favored).loyalty=clamp(npcRelation(n,favored).loyalty+2);recordSocialEvent(n,favored,'Took a side',`${n.name} quietly sided with ${favored.name} after conflict between ${a.name} and ${b.name}.`,1,{conflict:detail});});
}
function formSocialAlliance(a,b){
 ensureSocialNetwork();const key=[a.id,b.id].sort().join('|');if(world.socialNetwork.alliances.some(x=>x.key===key&&x.status==='active'))return;
 world.socialNetwork.alliances.push({id:uid('alliance'),key,aId:a.id,bId:b.id,status:'active',createdAt:worldDate(),settlementId:a.settlementId});recordSocialEvent(a,b,'Alliance formed',`${a.name} and ${b.name} have become reliable allies in local social life.`,2);
}
function formSocialFeud(a,b){
 ensureSocialNetwork();const key=[a.id,b.id].sort().join('|');if(world.socialNetwork.feuds.some(x=>x.key===key&&x.status==='active'))return;
 world.socialNetwork.feuds.push({id:uid('feud'),key,aId:a.id,bId:b.id,status:'active',createdAt:worldDate(),settlementId:a.settlementId});recordSocialEvent(a,b,'Feud formed',`${a.name} and ${b.name} now have an openly strained relationship.`,2);takeSidesInConflict(a,b,'ongoing feud');
}
function socialTriangleTick(n){
 if(!n.partnerId)return;const partner=world.npcs.find(x=>x.id===n.partnerId&&x.alive!==false);if(!partner)return;
 const friends=Object.entries(n.npcRelations||{}).map(([id,r])=>({other:world.npcs.find(x=>x.id===id),r})).filter(x=>x.other&&x.other.id!==partner.id&&x.r.affection>55&&x.r.closeness>45);
 if(!friends.length)return;const third=friends[Math.floor(Math.random()*friends.length)],pr=npcRelation(n,partner);
 if(third.r.affection>70&&Math.random()<.02){pr.jealousy=clamp(pr.jealousy+8);const partnerView=npcRelation(partner,third.other);partnerView.jealousy=clamp(partnerView.jealousy+6);recordSocialEvent(n,partner,'Jealousy surfaced',`${partner.name} became uncomfortable with how close ${n.name} and ${third.other.name} have become.`,1,{thirdId:third.other.id});}
}
function reconcileNpcConflict(a,b){
 const r=npcRelation(a,b),back=npcRelation(b,a);if(r.tension<25||Math.random()>.035)return false;
 const compatible=(r.respect+back.respect+r.trust+back.trust)>90;if(!compatible)return false;
 r.tension=clamp(r.tension-12);back.tension=clamp(back.tension-12);r.trust=clamp(r.trust+4);back.trust=clamp(back.trust+4);recordSocialEvent(a,b,'Reconciliation',`${a.name} and ${b.name} made an effort to repair their relationship.`,2);
 const key=[a.id,b.id].sort().join('|');world.socialNetwork.feuds.filter(x=>x.key===key&&x.status==='active').forEach(x=>x.status='resolved');return true;
}
function npcSocialNetworkTick(n){
 ensureSocialNetwork();ensureNpcLife(n);const peers=residentsOf(n.settlementId).filter(x=>x.id!==n.id&&x.alive!==false);if(!peers.length)return;
 Object.keys(n.npcRelations||{}).forEach(id=>{const other=world.npcs.find(x=>x.id===id);if(!other)return;const r=npcRelation(n,other);updateSharedGroups(n,other);syncNpcRelationType(n,other);if(r.type==='close friend'&&r.loyalty>45)formSocialAlliance(n,other);if(r.type==='enemy'&&r.tension>72)formSocialFeud(n,other);if(r.tension>35)reconcileNpcConflict(n,other);});
 socialTriangleTick(n);
 if(Math.random()<.025){const close=peers.filter(x=>npcRelation(n,x).trust>40);if(close.length){const subject=close[Math.floor(Math.random()*close.length)],r=npcRelation(n,subject),positive=r.affection>=r.tension;gossipAbout(n,subject,positive?`${subject.name} has been dependable lately.`:`${subject.name} has been difficult to deal with lately.`,{tone:positive?'positive':'negative',importance:1});}}
}
function simulateSocialNetworkDay(){
 if(!world)return;ensureSocialNetwork();const stamp=`${world.clock.year}-${world.clock.seasonIndex}-${world.clock.day}`;if(world.socialNetwork.lastDailyTick===stamp)return;world.socialNetwork.lastDailyTick=stamp;world.npcs.filter(n=>n.alive!==false).forEach(n=>npcSocialNetworkTick(n));
}
function npcNetworkLinks(n){
 return Object.entries(n.npcRelations||{}).map(([id,r])=>({other:world.npcs.find(x=>x.id===id),r})).filter(x=>x.other&&x.other.alive!==false).sort((a,b)=>(b.r.closeness+b.r.trust-b.r.tension)-(a.r.closeness+a.r.trust-a.r.tension));
}
function renderSocialNetwork(){
 if(!$('socialNetworkView')||!world)return;const ch=activeCharacter();$('noSocialNetworkLife').classList.toggle('hidden',!!ch);$('socialNetworkGame').classList.toggle('hidden',!ch);if(!ch)return;ensureSocialNetwork();const town=currentSettlement(ch),locals=residentsOf(town.id),alliances=world.socialNetwork.alliances.filter(x=>x.settlementId===town.id&&x.status==='active'),feuds=world.socialNetwork.feuds.filter(x=>x.settlementId===town.id&&x.status==='active');
 $('socialNetworkSummary').innerHTML=`<span>Residents <b>${locals.length}</b></span><span>Alliances <b>${alliances.length}</b></span><span>Feuds <b>${feuds.length}</b></span><span>Gossip records <b>${world.socialNetwork.gossip.filter(x=>x.settlementId===town.id).length}</b></span>`;
 $('socialNetworkPeople').innerHTML=locals.slice(0,18).map(n=>{const links=npcNetworkLinks(n).slice(0,4);return `<article class="network-card"><div><strong>${escapeHtml(n.name)}</strong><small>${escapeHtml(n.occupation||n.role)} · ${escapeHtml(n.autonomy?.maritalStatus||'single')}</small></div><div class="network-links">${links.map(x=>`<span>${escapeHtml(x.other.name)} · ${escapeHtml(x.r.type)}${x.r.tension>35?` · tension ${x.r.tension}`:''}</span>`).join('')||'<span>No strong local ties yet</span>'}</div></article>`}).join('')||'<p class="muted">No local social network is established yet.</p>';
 const rows=world.socialNetwork.events.filter(x=>x.settlementId===town.id).slice().reverse().slice(0,28);$('socialNetworkLog').innerHTML=rows.length?rows.map(x=>`<div class="event"><strong>${escapeHtml(x.kind)}</strong><div>${escapeHtml(x.detail)}</div><small>${escapeHtml(x.worldDate)}</small></div>`).join(''):'<p class="muted">No major social-network changes yet.</p>';
}

function autonomyDecisionLog(n,kind,detail,{importance=1,targetId=null}={}){
 ensureNpcSimulation();const row={id:uid('autonomy'),npcId:n.id,kind,detail,targetId,importance,settlementId:n.settlementId,worldDate:worldDate(),year:world.clock.year};
 world.npcAutonomy.decisions.push(row);if(world.npcAutonomy.decisions.length>500)world.npcAutonomy.decisions.shift();
 if(importance>=2){world.npcAutonomy.milestones.push(row);if(world.npcAutonomy.milestones.length>220)world.npcAutonomy.milestones.shift();}
 npcLifeLog(n,kind,detail);return row;
}
function npcOpinion(n,other){
 ensureNpcLife(n);n.autonomy.opinions[other.id]=n.autonomy.opinions[other.id]||{liking:0,respect:0,suspicion:0,lastUpdated:worldDate()};
 return n.autonomy.opinions[other.id];
}
function updateNpcOpinion(n,other,d){
 const o=npcOpinion(n,other);o.liking=clamp(o.liking+(d.liking||0),-100,100);o.respect=clamp(o.respect+(d.respect||0),-100,100);o.suspicion=clamp(o.suspicion+(d.suspicion||0),0,100);o.lastUpdated=worldDate();
}
function chooseNpcAutonomyAction(n){
 ensureNpcLife(n);const a=n.autonomy,choices=[];
 if(n.age<14)return 'childhood';
 if(a.stress>65)choices.push('rest','family','friend');
 if(a.ambition>55)choices.push('work','work','career');
 if(a.trainingDrive>55)choices.push('train');
 if(a.sociability>50)choices.push('friend','friend');
 if(a.familyDrive>60)choices.push('family');
 if(a.wanderlust>65)choices.push('travel');
 if(a.risk>65)choices.push('risk');
 if(n.money<6)choices.push('work','work');
 if(n.partnerId)choices.push('partner');
 choices.push('routine','routine');
 return choices[Math.floor(Math.random()*choices.length)];
}
function npcAutonomyWork(n){
 const a=n.autonomy,old=n.careerLevel;n.money+=2+n.careerLevel;a.skills.work=Math.min(100,a.skills.work+1);n.careerXp+=3+Math.floor(a.ambition/30);a.stress=clamp(a.stress+2);a.satisfaction=clamp(a.satisfaction+(a.ambition>55?1:0));
 if(n.careerXp>=(n.careerLevel+1)*95&&n.careerLevel<5){n.careerLevel++;autonomyDecisionLog(n,'Career advancement',`${n.name} advanced professionally as a ${n.occupation||n.role}.`,{importance:2});}
 else if(old===n.careerLevel&&Math.random()<.012&&a.satisfaction<30){const oldRole=n.occupation||n.role,newRole=townNpcRoles[Math.floor(Math.random()*townNpcRoles.length)];n.occupation=newRole;n.role=newRole;a.satisfaction=50;autonomyDecisionLog(n,'Changed careers',`${n.name} left work as ${oldRole} and began working as a ${newRole}.`,{importance:2});}
}
function npcAutonomyTrain(n){
 const a=n.autonomy;a.skills.combat=Math.min(100,a.skills.combat+1+Math.floor(a.trainingDrive/40));a.stress=clamp(a.stress-1);a.satisfaction=clamp(a.satisfaction+2);
 if(a.skills.combat%20<3&&Math.random()<.16)autonomyDecisionLog(n,'Training progress',`${n.name} has been steadily improving their martial or bending ability.`,{importance:1});
}
function npcAutonomySocial(n){
 const peers=residentsOf(n.settlementId).filter(x=>x.id!==n.id&&x.alive!==false);if(!peers.length)return;const other=peers[Math.floor(Math.random()*peers.length)],r=npcRelation(n,other),back=npcRelation(other,n),positive=Math.random()<(0.58+n.autonomy.sociability/300);
 if(positive){const gain=1+(n.autonomy.sociability>70?1:0);r.trust=clamp(r.trust+gain);r.affection=clamp(r.affection+gain);r.closeness=clamp(r.closeness+gain);r.loyalty=clamp(r.loyalty+1);back.trust=clamp(back.trust+1);back.affection=clamp(back.affection+1);back.loyalty=clamp(back.loyalty+1);updateNpcOpinion(n,other,{liking:2,respect:1});updateNpcOpinion(other,n,{liking:1});n.autonomy.satisfaction=clamp(n.autonomy.satisfaction+2);}
 else{r.tension=clamp(r.tension+3);back.tension=clamp(back.tension+2);r.loyalty=clamp(r.loyalty-1);back.loyalty=clamp(back.loyalty-1);updateNpcOpinion(n,other,{liking:-2,suspicion:2});updateNpcOpinion(other,n,{liking:-1,suspicion:1});if(r.tension>55&&Math.random()<.18){autonomyDecisionLog(n,'Argument',`${n.name} and ${other.name} had a serious disagreement.`,{importance:1,targetId:other.id});recordSocialEvent(n,other,'Argument',`${n.name} and ${other.name} had a serious disagreement.`,1);takeSidesInConflict(n,other,'argument');gossipAbout(n,other,`${n.name} is upset with ${other.name} after an argument.`,{tone:'negative'});}}syncNpcRelationType(n,other);
 if(!n.partnerId&&!other.partnerId&&n.age>=18&&other.age>=18&&r.trust>55&&r.affection>60&&r.closeness>50&&npcOpinion(n,other).liking>8&&Math.random()<.035){n.partnerId=other.id;other.partnerId=n.id;n.autonomy.maritalStatus='dating';other.autonomy.maritalStatus='dating';autonomyDecisionLog(n,'Relationship began',`${n.name} and ${other.name} began dating.`,{importance:2,targetId:other.id});autonomyDecisionLog(other,'Relationship began',`${other.name} and ${n.name} began dating.`,{importance:2,targetId:n.id});recordSocialEvent(n,other,'Dating began',`${n.name} and ${other.name} began dating.`,2);gossipAbout(n,other,`${n.name} and ${other.name} have started seeing each other.`,{tone:'positive',importance:2});}
}
function npcAutonomyPartner(n){
 const p=world.npcs.find(x=>x.id===n.partnerId&&x.alive!==false);if(!p){n.partnerId=null;n.autonomy.maritalStatus='single';return;}const r=npcRelation(n,p),back=npcRelation(p,n);
 if(r.tension>65&&Math.random()<.08){n.partnerId=null;p.partnerId=null;n.autonomy.maritalStatus='single';p.autonomy.maritalStatus='single';autonomyDecisionLog(n,'Relationship ended',`${n.name} and ${p.name} separated after sustained tension.`,{importance:2,targetId:p.id});recordSocialEvent(n,p,'Breakup',`${n.name} and ${p.name} separated after sustained tension.`,2);takeSidesInConflict(n,p,'breakup');gossipAbout(n,p,`${n.name} and ${p.name} have separated.`,{tone:'neutral',importance:2});return;}
 if(r.trust>72&&r.affection>72&&r.closeness>65&&n.age>=20&&p.age>=20&&n.autonomy.maritalStatus!=='married'&&Math.random()<.015){n.autonomy.maritalStatus='married';p.autonomy.maritalStatus='married';autonomyDecisionLog(n,'Marriage',`${n.name} and ${p.name} married.`,{importance:3,targetId:p.id});recordSocialEvent(n,p,'Marriage',`${n.name} and ${p.name} married.`,3);gossipAbout(n,p,`${n.name} and ${p.name} married.`,{tone:'positive',importance:2});world.events.push(makeEvent('A local marriage',`${n.name} and ${p.name} married in ${world.settlements.find(s=>s.id===n.settlementId)?.name||'their community'}.`,[n.id,p.id]));}
 else{r.trust=clamp(r.trust+1);r.affection=clamp(r.affection+1);back.trust=clamp(back.trust+1);n.autonomy.satisfaction=clamp(n.autonomy.satisfaction+2);}
}
function npcAutonomyFamily(n){
 const family=(n.familyIds||[]).map(id=>world.npcs.find(x=>x.id===id)).filter(x=>x&&x.alive!==false);if(!family.length){n.autonomy.satisfaction=clamp(n.autonomy.satisfaction-1);return;}const other=family[Math.floor(Math.random()*family.length)];updateNpcOpinion(n,other,{liking:2,respect:1});n.autonomy.stress=clamp(n.autonomy.stress-3);n.autonomy.satisfaction=clamp(n.autonomy.satisfaction+3);
}
function npcAutonomyTravel(n){
 const from=world.settlements.find(s=>s.id===n.settlementId),choices=world.settlements.filter(s=>s.id!==n.settlementId&&(s.homeNation===from?.homeNation||n.autonomy.wanderlust>80));if(!choices.length)return;const dest=choices[Math.floor(Math.random()*choices.length)];
 if(Math.random()<.12){const old=from?.name||'their former home';n.settlementId=dest.id;n.homeSettlementId=n.autonomy.stability<35?dest.id:n.homeSettlementId;n.lifeStatus='traveler';n.autonomy.satisfaction=clamp(n.autonomy.satisfaction+5);autonomyDecisionLog(n,n.homeSettlementId===dest.id?'Moved settlements':'Traveled',`${n.name} ${n.homeSettlementId===dest.id?'moved':'traveled'} from ${old} to ${dest.name}.`,{importance:n.homeSettlementId===dest.id?2:1});}
}
function npcAutonomyRisk(n){
 const a=n.autonomy;if(Math.random()<.35){a.skills.combat=Math.min(100,a.skills.combat+2);a.satisfaction=clamp(a.satisfaction+3);autonomyDecisionLog(n,'Took a risk',`${n.name} took a personal risk and came through it with more confidence.`,{importance:1});}
 else{a.stress=clamp(a.stress+8);n.health.vitality=clamp(n.health.vitality-(3+Math.floor(Math.random()*8)));if(Math.random()<.18){n.health.condition='injured';autonomyDecisionLog(n,'Injured',`${n.name} was injured while taking a risky action.`,{importance:2});}}
}
function npcAutonomyRoutine(n){n.autonomy.stress=clamp(n.autonomy.stress-2);n.autonomy.satisfaction=clamp(n.autonomy.satisfaction+(Math.random()<.5?1:0));}
function npcAutonomyDay(n){
 ensureNpcLife(n);const a=n.autonomy;if(a.decisionCooldown>0){a.decisionCooldown--;npcAutonomyRoutine(n);return;}const decision=chooseNpcAutonomyAction(n);a.lastDecision=decision;a.decisionCooldown=Math.random()<.2?1:0;
 if(decision==='work'||decision==='career')npcAutonomyWork(n);else if(decision==='train')npcAutonomyTrain(n);else if(decision==='friend')npcAutonomySocial(n);else if(decision==='partner')npcAutonomyPartner(n);else if(decision==='family')npcAutonomyFamily(n);else if(decision==='travel')npcAutonomyTravel(n);else if(decision==='risk')npcAutonomyRisk(n);else npcAutonomyRoutine(n);
 if(a.stress>80)a.satisfaction=clamp(a.satisfaction-3);if(a.satisfaction<20&&Math.random()<.03)n.goal=npcGoalPool[Math.floor(Math.random()*npcGoalPool.length)];
}
function simulateNpcAutonomyDay(){
 if(!world)return;ensureNpcSimulation();const stamp=`${world.clock.year}-${world.clock.seasonIndex}-${world.clock.day}`;if(world.npcAutonomy.lastDailyTick===stamp)return;world.npcAutonomy.lastDailyTick=stamp;
 world.npcs.filter(n=>n.alive!==false).forEach(n=>npcAutonomyDay(n));
}
function renderNpcAutonomy(){
 if(!$('npcAutonomyView')||!world)return;const ch=activeCharacter();$('noNpcAutonomyLife').classList.toggle('hidden',!!ch);$('npcAutonomyGame').classList.toggle('hidden',!ch);if(!ch)return;ensureNpcSimulation();const town=currentSettlement(ch),locals=residentsOf(town.id).slice().sort((a,b)=>(b.history?.length||0)-(a.history?.length||0)).slice(0,18);
 $('npcAutonomySummary').innerHTML=`<span>Local NPCs <b>${locals.length}</b></span><span>Autonomous decisions <b>${world.npcAutonomy.decisions.length}</b></span><span>Major life changes <b>${world.npcAutonomy.milestones.length}</b></span>`;
 $('npcAutonomyPeople').innerHTML=locals.map(n=>`<article class="autonomy-card"><div><strong>${escapeHtml(n.name)}</strong><small>${escapeHtml(n.occupation||n.role)} · ${n.age} · ${escapeHtml(n.lifeStatus||'resident')}</small></div><div class="autonomy-stats"><span>Goal: ${escapeHtml(n.goal||'ordinary life')}</span><span>Satisfaction ${n.autonomy.satisfaction}</span><span>Stress ${n.autonomy.stress}</span><span>Last choice: ${escapeHtml(n.autonomy.lastDecision||'routine')}</span></div></article>`).join('')||'<p class="muted">No simulated residents are established here yet.</p>';
 const rows=world.npcAutonomy.decisions.filter(x=>x.settlementId===town.id).slice().reverse().slice(0,24);$('npcAutonomyLog').innerHTML=rows.length?rows.map(x=>`<div class="event"><strong>${escapeHtml(personById(x.npcId)?.name||'Resident')} · ${escapeHtml(x.kind)}</strong><div>${escapeHtml(x.detail)}</div><small>${escapeHtml(x.worldDate)}</small></div>`).join(''):'<p class="muted">No autonomous local life changes recorded yet.</p>';
}
...
function simulateNpcYear(){
 ...
 world.npcs.slice().forEach(n=>{if(!n.alive)return;ensureNpcLife(n);n.age=Math.max(n.age+1,world.clock.year-n.birthYear);
 if(n.age>=14&&n.occupation==='Child'){n.occupation=inheritNpcCareer(n)||townNpcRoles[Math.floor(Math.random()*townNpcRoles.length)];n.role=n.occupation;n.lifeStatus='resident';n.goal=npcGoalPool[Math.floor(Math.random()*npcGoalPool.length)];autonomyDecisionLog(n,'Came of age',`${n.name} began learning the work of a ${n.occupation}.`,{importance:2});}
 if(n.partnerId&&n.autonomy.maritalStatus==='married'&&n.age>=20&&n.age<=45&&n.childrenIds.length<4&&n.autonomy.familyDrive>45&&Math.random()<.14){const p=world.npcs.find(x=>x.id===n.partnerId);if(p&&p.alive&&n.id<p.id){const child=makeNpcChild(n,p);if(child)world.npcAutonomy.milestones.push({id:uid('autonomy'),npcId:n.id,kind:'Child born',detail:`${n.name} and ${p.name} welcomed ${child.name}.`,importance:3,settlementId:n.settlementId,worldDate:worldDate()});}}
 if(n.health.condition==='injured'&&Math.random()<.65){n.health.condition='well';n.health.vitality=clamp(n.health.vitality+18);autonomyDecisionLog(n,'Recovered',`${n.name} recovered from an earlier injury.`,{importance:1});}
 n.autonomy.stress=clamp(n.autonomy.stress-8);n.autonomy.satisfaction=clamp(n.autonomy.satisfaction+(Math.random()<.5?2:-1));
 const mortality=n.age<55?.001:n.age<70?.012:n.age<85?.05:.16;if(Math.random()<mortality){n.alive=false;n.active=false;n.lifeStatus='deceased';n.health.condition='deceased';autonomyDecisionLog(n,'Died',`${n.name} died at age ${n.age}. Their relationships and history remain part of the world.`,{importance:3});world.events.push(makeEvent('A life ends',`${n.name}, ${n.occupation||n.role}, died at age ${n.age}.`,[n.id]));historicalMemoryFromEvent({title:'A life ends',detail:`${n.name}, ${n.occupation||n.role}, died at age ${n.age}.`,settlementId:n.settlementId,participantIds:[n.id],importance:2});transferNpcLegacyOnDeath(n);}});
}
...
function ensurePersonalStory(ch){
 world.personalStoryState=world.personalStoryState||{version:1,characters:{}};const all=world.personalStoryState.characters;
 if(all[ch.id])return all[ch.id];
 const family=(ch.familyIds||[]).map(id=>world.npcs.find(n=>n.id===id)).filter(Boolean),rels=relationshipPeople(ch).filter(x=>x.n),hook=ch.backgroundHook||'',wish=ch.storyWish||'Surprise me';
 const threads=[];
 const add=(title,type,premise,question,heat=55)=>threads.push({id:uid('story'),title,type,premise,question,heat,status:'active',stage:'introduction',beats:[],created:worldDate(),lastAdvanced:null});
 if(hook)add('The Past Is Not Finished','personal',hook,'What is really unresolved here, and what will it cost to face it?',82);
 if(family.length)add('Family Ties','family',`${family[0].name} and the rest of ${ch.name}'s family have needs, expectations, and lives of their own.`,`How will ${ch.name}'s choices change the family?`,62);
 if(ch.goal)add('The Life I Want','ambition',ch.goal,`What is ${ch.name} willing to do—and give up—to reach this goal?`,70);
 if(ch.fear)add('What I Avoid','internal',ch.fear,`When will ${ch.name} have to confront this fear rather than simply avoid it?`,45);
 if(rels.length)add('Someone Who Matters','relationship',`${rels[0].n.name} already matters in ${ch.name}'s life.`,`What will this relationship become as both lives change?`,58);
 const state={characterId:ch.id,storyWish:wish,focus:ch.storyFocus||{},threads:threads.slice(0,5),chapter:1,currentScene:null,sceneHistory:[],canonTruths:[],openQuestions:threads.map(x=>x.question),lastSceneStamp:null};
 all[ch.id]=state;return state;
}
function personalStoryFor(ch){return ensurePersonalStory(ch);}
function chooseStoryThread(ch){
 const ps=personalStoryFor(ch),active=ps.threads.filter(x=>x.status==='active');if(!active.length)return null;
 return active.slice().sort((a,b)=>b.heat-a.heat)[0];
}
function buildPlayableScene(ch,{force=false}={}){
 const ps=personalStoryFor(ch),stamp=`${world.clock.year}-${world.clock.seasonIndex}-${world.clock.day}-${Math.floor(world.clock.hour/3)}`;if(!force&&ps.currentScene&&ps.lastSceneStamp===stamp)return ps.currentScene;
 const town=currentSettlement(ch),place=town.places.find(p=>p.key===ch.currentLocationKey)||town.places[0],present=presentNpcs(town,ch.currentLocationKey),thread=chooseStoryThread(ch),localEvent=(world.everydayEvents?.events||[]).find(e=>e.settlementId===town.id&&e.status==='active'),major=(world.worldEvents2?.events||[]).find(e=>e.settlementId===town.id&&e.status!=='resolved');
 let setup=`${world.clock.hour<11?'Morning':world.clock.hour<17?'Afternoon':'Evening'} in ${town.name}. ${ch.name} is at ${place.name}. ${place.desc||''}`.trim();
 let pressure='For the moment, life is moving at its ordinary pace.';
 if(thread?.type==='personal')pressure=`Something from ${ch.name}'s unfinished past is close enough to matter again: ${thread.premise}`;
 else if(thread?.type==='family')pressure=`Family life is not standing still: ${thread.premise}`;
 else if(thread?.type==='ambition')pressure=`The day presents another small chance to move toward what ${ch.name} wants: ${thread.premise}`;
 else if(localEvent)pressure=`Nearby, ${localEvent.title} is drawing attention. It is happening whether ${ch.name} gets involved or not.`;
 else if(major)pressure=`People around town are reacting to ${major.title}, though nobody is waiting for ${ch.name} to solve it.`;
 if(present[0])pressure+=` ${present[0].name} is here and appears occupied with their own day.`;
 const actions=[];
 if(thread?.type==='personal')actions.push({label:'Follow the thread from my past',text:`I pay attention to anything connected to this unresolved part of my past: ${thread.premise}`});
 if(thread?.type==='family')actions.push({label:'Check in with my family',text:'I spend time with my family and see what is going on in their lives today.'});
 if(thread?.type==='ambition')actions.push({label:'Work toward my goal',text:`I take a practical step toward my goal: ${ch.goal}`});
 if(present[0])actions.push({label:`Talk to ${present[0].name}`,text:`I talk to ${present[0].name} and see what is on their mind.`});
 if(localEvent)actions.push({label:`Check out ${localEvent.title}`,text:`I go see what is happening with ${localEvent.title}.`});
 actions.push({label:'Continue my normal day',text:ch.occupation&&ch.occupation!=='Unemployed / between paths'?`I get on with my normal responsibilities as a ${ch.occupation}.`:'I go about my normal day and see what develops.'});
 actions.push({label:'Explore instead',text:'I head out and explore the area to see what catches my attention.'});
 ps.currentScene={id:uid('scene'),chapter:ps.chapter,title:thread?thread.title:`Life in ${town.name}`,setup,pressure,threadId:thread?.id||null,actions:actions.slice(0,5),worldDate:worldDate(),hour:world.clock.hour,location:place.name};ps.lastSceneStamp=stamp;ps.sceneHistory.push({...ps.currentScene,actions:undefined});ps.sceneHistory=ps.sceneHistory.slice(-100);return ps.currentScene;
}
function storyAction(text){
 const input=$('gmActionInput');if(!input)return;input.value=text;const details=document.querySelector('.freeform-advanced');if(details)details.open=true;submitFreeformAction();
}
function renderPlayScene(){
 const box=$('playScene');if(!box||!world)return;const ch=activeCharacter();if(!ch)return;const ps=personalStoryFor(ch),scene=buildPlayableScene(ch);
 box.innerHTML=`<div class="story-chapter"><span>CHAPTER ${scene.chapter}</span><span>${escapeHtml(scene.worldDate)} · ${String(scene.hour).padStart(2,'0')}:00</span></div><h2>${escapeHtml(scene.title)}</h2><p class="scene-prose">${escapeHtml(scene.setup)}</p><p class="scene-prose"><strong>${escapeHtml(scene.pressure)}</strong></p><div class="story-actions">${scene.actions.map((a,i)=>`<button data-story-action="${i}" ${i===0?'class="primary"':''}>${escapeHtml(a.label)}</button>`).join('')}<button data-story-custom="1">Do something else…</button></div>`;
 box.querySelectorAll('[data-story-action]').forEach(b=>b.onclick=()=>storyAction(scene.actions[Number(b.dataset.storyAction)].text));
 box.querySelector('[data-story-custom]')?.addEventListener('click',()=>{const d=document.querySelector('.freeform-advanced');if(d)d.open=true;$('gmActionInput')?.focus();$('gmActionInput')?.scrollIntoView({behavior:'smooth',block:'center'});});
}
function renderMyStory(){
 const el=$('myStoryPanel');if(!el||!world)return;const ch=activeCharacter();if(!ch)return;const ps=personalStoryFor(ch);
 el.innerHTML=`<p class="eyebrow">MY STORY · DIRECTOR-GROWN, NOT PREWRITTEN</p><h3>${escapeHtml(ps.storyWish)}</h3>${ps.threads.map(x=>`<div class="story-thread"><div><strong>${escapeHtml(x.title)}</strong><span>${escapeHtml(x.type)} · ${escapeHtml(x.stage)}</span></div><p>${escapeHtml(x.premise)}</p><small>${escapeHtml(x.question)}</small></div>`).join('')}`;
}
...
function renderFirstDay(){
 ...
 $('firstDayChoices').innerHTML=`<button class="primary" data-firstday="life">Start My First Day</button><button data-firstday="work">${ch.occupation==='Unemployed / between paths'?'Look for work':'Start with work'}</button>${d.friend?'<button data-firstday="friend">Find someone I know</button>':''}<button data-firstday="events">See what is happening today</button>`;
...
function beginFirstDayChoice(kind){
 ...
 try{
   ...
   else {openTab('lifeView');setTimeout(()=>document.getElementById('howToPlayPanel')?.scrollIntoView({behavior:'smooth',block:'start'}),50);}
...
function storyDirectorContext(ctx){
 const {ch,town,place,present,targetNpc}=ctx;ensureFactionState();ensureEconomyState();ensureSpiritState();ensureNpcSimulation();world.director2=world.director2||{dailyScenes:{},latency:[],lastFallback:null};
 const economy=settlementEconomy(town.id),rels=relationshipPeople(ch).sort((a,b)=>(b.r.closeness||b.r.trust||0)-(a.r.closeness||a.r.trust||0)).slice(0,4);
 const target=targetNpc?{name:targetNpc.name,role:targetNpc.role||targetNpc.occupation,personality:(targetNpc.personality||[]).slice?.(0,4)||targetNpc.personality,goal:targetNpc.goal,relationship:(()=>{const r=relationshipWith(ch,targetNpc);return {type:r.type,trust:r.trust,respect:r.respect,affection:r.affection,tension:r.tension};})(),knowledge:npcKnowledgeContext(targetNpc,ch),sharedHistory:sharedHistoryContext(ch,targetNpc,ctx.rawAction)}:null;
 return {era:world.era,date:worldDate(),hour:world.clock.hour,weather:world.environment.weather,location:{settlement:town.name,place:place.name,description:place.desc},character:{name:ch.name,age:ch.age,nation:ch.nation,occupation:ch.occupation,bending:{element:ch.bending?.element,training:ch.bending?.training},needs:ch.needs,currency:ch.currency,training:ch.training?{mastery:ch.training.mastery,style:ch.training.style}:null,backgroundHook:ch.backgroundHook||null,storyWish:ch.storyWish||'Surprise me',storyFocus:ch.storyFocus||{}},target,nearby:present.slice(0,5).map(n=>({name:n.name,role:n.role||n.occupation,goal:n.goal})),closeRelationships:rels.map(x=>({name:x.n.name,type:x.r.familyRole||x.r.type,trust:x.r.trust,tension:x.r.tension})),opportunities:(world.opportunities||[]).filter(o=>o.settlementId===town.id&&o.status==='open').slice(0,3).map(o=>({title:o.title,description:o.description,locationKey:o.locationKey})),recentEvents:(world.events||[]).slice(-3).map(e=>({title:e.title,detail:e.detail||e.description})),majorLocalEvents:(world.worldEvents2?.events||[]).filter(e=>e.settlementId===town.id&&e.status!=='resolved').slice(0,2).map(e=>({title:e.title,type:e.type,status:e.status,progress:e.progress,severity:e.severity,detail:e.detail})),personalStory:(()=>{const p=personalStoryFor(ch),th=chooseStoryThread(ch);return {wish:p.storyWish,currentThread:th?{title:th.title,type:th.type,premise:th.premise,question:th.question,stage:th.stage}:null,canonTruths:p.canonTruths.slice(-8),openQuestions:p.openQuestions.slice(-6)};})(),questlessThreads:questlessContext(ch),generationalContext:generationalContext(ch),settlementEvolution:settlementEvolutionContext(town),culturalContext:culturalContext(town),recentActions:(world.storyDirector?.memory||[]).filter(m=>m.characterId===ch.id).slice(-3).map(m=>({action:m.action,outcome:m.outcome})),localConditions:{prosperity:economy.prosperity,shortages:economyGoods.filter(g=>economy.stocks[g.id]<18).slice(0,2).map(g=>g.name)},content:{combatIntensity:world.uiSettings?.combatIntensity,romance:world.uiSettings?.romance}};
}
...
function enrichLocalNarration(ctx,base){
 ...
 if(npc){const rel=relationshipWith(ctx.ch,npc),known=(npc.knowledge?.rumors||[]).length;parts.push(`${npc.name}'s reaction is filtered through ${rel.trust||0} trust, ${rel.respect||0} respect, and what they actually know${known?` (${known} remembered rumor${known===1?'':'s'})`:''}.`);}if(npc){const recalled=relevantNpcMemories(npc,ctx.ch,ctx.rawAction)[0];if(recalled&&recalled.relevance>=90)parts.push(`Something from your shared history still colors the moment: ${recalled.detail}`);}
...
async function connectedNarration(context){
  const endpoint=world.gmSettings?.endpoint||'/api/gm',timeoutMs=Math.max(5000,Math.min(30000,Number(world.gmSettings?.timeoutMs||18000))),started=performance.now();
  const payload={version:2,requestType:'narrate_resolved_action',context:{...storyDirectorContext(context),action:context.rawAction,resolution:{intent:context.parsed.intent,outcome:context.outcome.tier,label:context.outcome.label,allowed:context.feasibility.allowed,reasons:context.feasibility.reasons,score:context.score,difficulty:context.feasibility.difficulty},directives:['Do not override the resolved outcome.','Do not grant unearned powers, items, knowledge, relationships, or canon importance.','Keep the player an ordinary person in the world unless simulation state establishes otherwise.','NPCs may only reveal information in their supplied personal memory/knowledge or plausible local public knowledge. They are not omniscient.','NPC dialogue and emotional tone must reflect relevant supplied memories, shared experiences, promises, favors, insults, fights, romance, breakups, witnessed behavior, rumors, and suspected lies when they matter.','Do not mention memories mechanically. Use them as natural subtext, callbacks, changed trust, hesitation, warmth, resentment, or specific references.','Never let an NPC remember an event they did not witness, experience, or learn about through the supplied memory source.','Respect supplied content settings. Adult intimate content remains non-explicit.','Return narration only.']}};
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeoutMs);
  try{
    ...
    const elapsed=Math.round(performance.now()-started);world.director2=world.director2||{dailyScenes:{},latency:[],lastFallback:null};world.director2.latency.push({ms:elapsed,worldDate:worldDate(),type:'action'});world.director2.latency=world.director2.latency.slice(-40);return data.narration.slice(0,2600);
  }finally{clearTimeout(timer);}
}
...
function directorDayKey(ch){return `${world.clock.year}-${world.clock.seasonIndex}-${world.clock.day}-${ch.id}`;}
function morningSceneData(ch,town,place){
 ensureCharacterLifeState(ch);world.director2=world.director2||{dailyScenes:{},latency:[],lastFallback:null};const present=presentNpcs(town,ch.currentLocationKey),rels=relationshipPeople(ch),opps=(world.opportunities||[]).filter(o=>o.settlementId===town.id&&o.status==='open').slice(0,2),events=(world.everydayEvents?.events||[]).filter(e=>e.settlementId===town.id&&e.status==='active').slice(0,2),friend=rels.find(x=>(x.r.type==='friend'||x.r.type==='rival-friend')&&x.n),family=rels.find(x=>x.r.type==='family'&&x.n),bits=[];
 bits.push(`${world.environment.weather} weather settles over ${town.name} as ${ch.name}'s day begins at ${place.name}.`);
 if(family)bits.push(`${family.n.name} is already part of the morning rhythm nearby.`);
 if(friend)bits.push(`${friend.n.name} is someone ${ch.name} could reasonably seek out today.`);
 if(ch.occupation&&ch.occupation!=='Unemployed / between paths')bits.push(`Work as ${ch.occupation} is one ordinary responsibility waiting today.`);
 if(events[0])bits.push(`${events[0].title||events[0].name||'Something local'} is happening around town.`);
 if(opps[0])bits.push(`Word around ${town.name} points to a small local concern: ${opps[0].title}.`);const major=(world.worldEvents2?.events||[]).find(e=>e.settlementId===town.id&&e.status!=='resolved');if(major)bits.push(`A larger situation is developing in the community: ${major.title}. It will continue whether or not ${ch.name} gets involved.`);const personal=questlessContext(ch)[0];if(personal&&personal.attention>=45)bits.push(`Because of choices you've already made, ${personal.label} is becoming part of your ongoing life—but there is no required objective.`);
 if(ch.backgroundHook)bits.push(`In the background, ${ch.name} still carries an unfinished personal thread: ${ch.backgroundHook}.`);
 const suggestions=[];
 if(ch.occupation&&ch.occupation!=='Unemployed / between paths')suggestions.push({label:'Go to work',action:`I get ready and head to work as ${ch.occupation}.`});
 if(friend)suggestions.push({label:`Find ${friend.n.name}`,action:`I go find ${friend.n.name} and see how their morning is going.`});
 if(events[0])suggestions.push({label:'Check the local event',action:`I head toward ${events[0].title||events[0].name||'the local activity'} to see what is happening.`});
 if(opps[0])suggestions.push({label:'Follow the rumor',action:`I ask around about ${opps[0].title} and see whether I can learn what is going on.`});
 suggestions.push({label:'Take in the neighborhood',action:'I step outside and take in the neighborhood, paying attention to the people and anything unusual happening around me.'});
 return {text:bits.join(' '),suggestions:suggestions.slice(0,5),present};
}
function ensureMorningScene(ch,town,place){
 const key=directorDayKey(ch);world.director2=world.director2||{dailyScenes:{},latency:[],lastFallback:null};if(!world.director2.dailyScenes[key])world.director2.dailyScenes[key]={...morningSceneData(ch,town,place),createdAt:new Date().toISOString()};return world.director2.dailyScenes[key];
}
function renderDirectorGuidance(ch,town,place){
 if(!$('directorScene'))return;const scene=ensureMorningScene(ch,town,place);$('directorScene').innerHTML=`<p>${escapeHtml(scene.text)}</p>`;$('directorSuggestions').innerHTML=scene.suggestions.map((x,i)=>`<button data-director-action="${i}"><strong>${escapeHtml(x.label)}</strong></button>`).join('')+`<button data-director-custom="1"><strong>Do something else…</strong></button>`;
 $('directorSuggestions').querySelectorAll('[data-director-action]').forEach(b=>b.onclick=()=>{const x=scene.suggestions[Number(b.dataset.directorAction)];$('gmActionInput').value=x.action;$('gmActionInput').focus();});
 $('directorSuggestions').querySelector('[data-director-custom]')?.addEventListener('click',()=>{$('gmActionInput').focus();});
}
function shouldUseFastLocalNarration(ctx){
 const simple=['rest','eat','drink','meditate','travel','work'];return simple.includes(ctx.parsed.intent)&&!ctx.targetNpc&&!ctx.rawAction.match(/\b(secret|investigate|follow|confront|ask|why|who|mystery|spirit|attack|fight)\b/i);
}

async function submitFreeformAction(){
 const input=$('gmActionInput'),btn=$('gmSubmitBtn'),status=$('gmGenerationStatus');if(!input||!world||!btn)return;const rawAction=input.value.trim();if(!rawAction)return;
 if(btn.dataset.busy==='1')return;btn.dataset.busy='1';btn.disabled=true;btn.textContent='Resolving…';if(status){status.textContent='Resolving your action…';status.classList.remove('hidden');}
 let stage='starting',watchdog=null;
 try{
   const ch=activeCharacter(),town=currentSettlement(ch);if(!ch||!town)throw new Error('No playable character/location is active.');
   const place=town.places?.find(p=>p.key===ch.currentLocationKey)||town.places?.[0];if(!place)throw new Error('Current location could not be resolved.');
   stage='reading scene';renderDirectorGuidance(ch,town,place);const present=presentNpcs(town,ch.currentLocationKey),parsed=inferIntent(rawAction),targetNpc=findMentionedNpc(rawAction,present),feasibility=checkFeasibility(ch,town,place,present,parsed),score=relevantSkillScore(ch,parsed.intent),outcome=feasibility.allowed?rollOutcome(score,feasibility.difficulty):{tier:'failure',label:'Not possible as attempted',roll:null};
   const context={rawAction,ch,town,place,present,parsed,targetNpc,feasibility,score,outcome};
   if(targetNpc&&parsed.intent==='talk')recordConversationMemory(ch,targetNpc,rawAction,outcome.label);
   witnessPlayerAction(ch,town,place,rawAction,outcome,targetNpc);
   let narration='',provider='local';
   if(feasibility.allowed){
     stage='applying consequences';const hours=applyFreeformConsequences(context);context.hours=hours;
     if(world.gmSettings?.mode==='connected'&&!shouldUseFastLocalNarration(context)){
       stage='connected narration';if(status)status.textContent='The Story Director is narrating…';
       const hardTimeout=new Promise((_,reject)=>{watchdog=setTimeout(()=>{const e=new Error('Connected narration exceeded 25 seconds.');e.name='NarrationTimeout';reject(e);},25000);});
       try{narration=await Promise.race([connectedNarration(context),hardTimeout]);provider='connected';}
       catch(err){provider='local-fallback';world.director2=world.director2||{dailyScenes:{},latency:[],lastFallback:null};world.director2.lastFallback={worldDate:worldDate(),reason:err?.message||String(err)};if(status)status.textContent='AI was too slow or unavailable. Continuing with local narration…';}
       finally{if(watchdog){clearTimeout(watchdog);watchdog=null;}}
     }else if(world.gmSettings?.mode==='connected')provider='fast-local';
   }
   stage='building narration';if(!narration)narration=enrichLocalNarration(context,localNarration(context));
   const memoryCallback=memoryCallbackFor(context);recordMemoryCallback(context,memoryCallback);detectSharedMoment(context,narration);recognizeQuestlessStory(context,narration);const directorRewardEarned=feasibility.allowed?directorReward(context):null;rememberDirectorAction(context,narration,directorRewardEarned);const ps=personalStoryFor(ch),activeThread=chooseStoryThread(ch);if(activeThread){activeThread.beats.push({worldDate:worldDate(),action:rawAction,outcome:outcome.label});activeThread.beats=activeThread.beats.slice(-30);activeThread.lastAdvanced=worldDate();activeThread.heat=clamp(activeThread.heat+(outcome.tier==='success'?2:1));if(activeThread.beats.length>=3&&activeThread.stage==='introduction')activeThread.stage='development';if(activeThread.beats.length>=7&&activeThread.stage==='development')activeThread.stage='complication';}ps.currentScene=null;
   const entry={id:uid('gm'),characterId:ch.id,rawAction,intent:parsed.intent,outcome:outcome.tier,outcomeLabel:outcome.label,narration,provider,worldDate:worldDate(),hour:world.clock.hour,locationKey:ch.currentLocationKey,reward:directorRewardEarned?.label||null,debug:{score,difficulty:feasibility.difficulty,reasons:feasibility.reasons}};
   world.gmLog=world.gmLog||[];world.gmLog.push(entry);if(world.gmLog.length>250)world.gmLog=world.gmLog.slice(-250);addLifeLog(ch,`Freeform: ${outcome.label}`,narration);input.value='';
   stage='advancing time';if(feasibility.allowed)advanceHours(context.hours||1);else saveWorld();
   stage='rendering';renderGmPanel();
 }catch(err){
   console.error('Freeform action failed at stage:',stage,err);world.hotfixState=world.hotfixState||{version:'Beta 2 Hotfix 1',freeformFailures:[],tutorialSeen:false};world.hotfixState.freeformFailures.push({id:uid('failure'),stage,message:err?.message||String(err),worldDate:worldDate(),action:rawAction.slice(0,240)});world.hotfixState.freeformFailures=world.hotfixState.freeformFailures.slice(-40);
   try{const ch=activeCharacter();if(ch){const fallback=`Something interrupted the full action resolver, but ${ch.name}'s attempt is not lost: "${rawAction}". The game recovered at the ${stage} stage. You can try again or phrase the action differently.`;world.gmLog=world.gmLog||[];world.gmLog.push({id:uid('gm'),characterId:ch.id,rawAction,intent:'recovery',outcome:'system-recovery',outcomeLabel:'Recovered from an action error',narration:fallback,provider:'recovery',worldDate:worldDate(),hour:world.clock.hour,locationKey:ch.currentLocationKey});renderGmPanel();}}catch(inner){console.error('Recovery narration failed',inner);}
   if(status){status.textContent=`Recovered from an error during ${stage}. You can keep playing.`;status.classList.remove('hidden');}
 }finally{
   if(watchdog)clearTimeout(watchdog);btn.dataset.busy='0';btn.disabled=false;btn.textContent='Attempt Action';setTimeout(()=>{if(status)status.classList.add('hidden');},1800);
 }
}
...
function ensureGuidanceState(ch){
 world.guidanceState=world.guidanceState||{version:2,characters:{},mode:'guided'};world.guidanceState.characters=world.guidanceState.characters||{};
 if(!world.guidanceState.characters[ch.id])world.guidanceState.characters[ch.id]={stage:0,completed:[],dismissed:false,firstDayComplete:false,lastHint:null};
 return world.guidanceState.characters[ch.id];
}
function guidanceSceneSummary(ch,town,place){
 const present=presentNpcs(town,ch.currentLocationKey),event=(world.everydayEvents?.events||[]).find(e=>e.settlementId===town.id&&e.status==='active'),major=(world.worldEvents2?.events||[]).find(e=>e.settlementId===town.id&&e.status!=='resolved'),known=relationshipPeople(ch).filter(x=>x.n?.alive!==false);
 const lines=[`You are at ${place.name} in ${town.name}. It is ${String(world.clock.hour).padStart(2,'0')}:00 and the weather is ${world.environment.weather}.`];
 if(ch.occupation&&ch.occupation!=='Unemployed / between paths')lines.push(`You currently work as ${ch.occupation}. You can work when you want income or career progress.`);
 if(present.length)lines.push(`${present.length===1?present[0].name:`${present.length} people`} ${present.length===1?'is':'are'} nearby and available to interact with.`);
 else if(known.length)lines.push(`Nobody familiar is standing here right now, but you know people in the area and can seek them out.`);
 if(event)lines.push(`A smaller local event is happening today: ${event.title}.`);
 if(major)lines.push(`A larger situation is developing nearby: ${major.title}. It will continue whether you get involved or not.`);
 return lines;
}
function guidanceSteps(ch,town,place){
 const present=presentNpcs(town,ch.currentLocationKey),known=relationshipPeople(ch).filter(x=>x.n?.alive!==false),event=(world.everydayEvents?.events||[]).find(e=>e.settlementId===town.id&&e.status==='active');
 return [
  {id:'orient',title:'Look around',why:'Start by learning where you are and what is happening immediately around you.',button:'Take in my surroundings',action:'orient'},
  {id:'people',title:'Interact with someone',why:'Relationships are a major part of the game. Talking to people creates memories, trust, tension, friendships, romance, rumors, and story threads.',button:present.length?`Talk to ${present[0].name}`:known.length?`Find ${known[0].n.name}`:'Look for someone to talk to',action:'people'},
  {id:'responsibility',title:'Do something ordinary',why:'Your character has needs, money, work, training, and daily responsibilities. Ordinary choices are how the world starts becoming your story.',button:ch.occupation&&ch.occupation!=='Unemployed / between paths'?'Go to work':'Handle something practical',action:'responsibility'},
  {id:'world',title:'Engage with the world',why:'Explore, attend local events, travel, train, or follow something that interests you. You are never required to solve what is happening.',button:event?`Check out ${event.title}`:'Explore the area',action:'world'},
  {id:'freeform',title:'Try something completely your own',why:'Once you understand the basics, Freeform Action lets you describe any reasonable attempt in plain language.',button:'Show me Freeform Action',action:'freeform'}
 ];
}
function guidanceComplete(ch,id){
 const g=ensureGuidanceState(ch);if(!g.completed.includes(id))g.completed.push(id);const steps=['orient','people','responsibility','world','freeform'];g.stage=Math.min(steps.length,steps.findIndex(x=>!g.completed.includes(x))===-1?steps.length:steps.findIndex(x=>!g.completed.includes(x)));if(g.stage>=4)g.firstDayComplete=true;saveWorld();
}
function guidedAction(kind){
 const ch=activeCharacter(),town=currentSettlement(ch);if(!ch||!town)return;const place=town.places.find(p=>p.key===ch.currentLocationKey)||town.places[0],present=presentNpcs(town,ch.currentLocationKey),known=relationshipPeople(ch).filter(x=>x.n?.alive!==false);
 if(kind==='orient'){
   addLifeLog(ch,'Paused to take in the surroundings',`${ch.name} took a moment at ${place.name} to notice the weather, people nearby, and the rhythm of ${town.name}.`);guidanceComplete(ch,'orient');renderLife();return;
 }
 if(kind==='people'){
   let npc=present[0]||known[0]?.n||null;
   if(npc){guidanceComplete(ch,'people');startDialogue(npc.id);return;}
   createOpportunity(town);addLifeLog(ch,'Looked for company',`${ch.name} spent some time around ${town.name}, looking for someone approachable to talk with.`);advanceHours(1);guidanceComplete(ch,'people');return;
 }
 if(kind==='responsibility'){
   guidanceComplete(ch,'responsibility');
   if(ch.occupation&&ch.occupation!=='Unemployed / between paths'){openTab('careerView');return;}
   doLocalAction(ch.needs.hunger>55?'eat':ch.needs.energy<45?'rest':'explore');return;
 }
 if(kind==='world'){
   guidanceComplete(ch,'world');const event=(world.everydayEvents?.events||[]).find(e=>e.settlementId===town.id&&e.status==='active');if(event){openTab('eventsView');return;}doLocalAction('explore');return;
 }
 if(kind==='freeform'){
   guidanceComplete(ch,'freeform');const box=$('gmActionInput');if(box){box.scrollIntoView({behavior:'smooth',block:'center'});box.focus();}return;
 }
}
function guidanceQuickAction(kind){
 const ch=activeCharacter();if(!ch)return;
 if(kind==='people'){guidedAction('people');return;}
 if(kind==='work'){openTab('careerView');return;}
 if(kind==='train'){openTab('trainingView');return;}
 if(kind==='explore'){doLocalAction('explore');return;}
 if(kind==='travel'){openTab('travelView');return;}
 if(kind==='events'){openTab('eventsView');return;}
 if(kind==='rest'){doLocalAction('rest');return;}
 if(kind==='eat'){doLocalAction('eat');return;}
 if(kind==='relationships'){openTab('relationshipsView');return;}
 if(kind==='stories'){openTab('questlessView');return;}
 if(kind==='freeform'){guidedAction('freeform');return;}
}
function renderHowToPlay(){
 const panel=$('howToPlayPanel');if(!panel||!world)return;const ch=activeCharacter();if(!ch)return;const town=currentSettlement(ch),place=town.places.find(p=>p.key===ch.currentLocationKey)||town.places[0],g=ensureGuidanceState(ch),steps=guidanceSteps(ch,town,place),current=steps.find(x=>!g.completed.includes(x.id))||steps[steps.length-1],summary=guidanceSceneSummary(ch,town,place);
 panel.innerHTML=`<div class="play-guide-head"><div><p class="eyebrow">START HERE · GUIDED PLAY</p><h2>${g.firstDayComplete?'What do you want to do next?':'Your first day, one step at a time'}</h2><p class="muted">${g.firstDayComplete?'Use any category below, follow something happening in the world, or use Freeform when you want complete freedom.':'You do not need to understand every tab. Follow the highlighted step and the game will teach itself as you play.'}</p></div><span class="status-pill">${Math.min(g.completed.length,5)}/5 basics</span></div>
 <div class="guidance-scene">${summary.map(x=>`<p>${escapeHtml(x)}</p>`).join('')}</div>
 ${!g.firstDayComplete?`<div class="next-step-card"><p class="eyebrow">RECOMMENDED NEXT STEP</p><h3>${escapeHtml(current.title)}</h3><p>${escapeHtml(current.why)}</p><button class="primary" data-guide-main="${current.action}">${escapeHtml(current.button)}</button></div>`:''}
 <details class="guide-details" ${g.firstDayComplete?'open':''}><summary>What can I do in this game?</summary>
 <div class="guide-action-grid">
  <button data-guide-quick="people"><strong>Talk & Relationships</strong><small>Meet people, build friendships, date, argue, reconcile.</small></button>
  <button data-guide-quick="work"><strong>Work & Career</strong><small>Earn money, advance your profession, build a business.</small></button>
  <button data-guide-quick="train"><strong>Train</strong><small>Practice bending, martial arts, and learned techniques.</small></button>
  <button data-guide-quick="explore"><strong>Explore Nearby</strong><small>Spend time looking around and discovering local opportunities.</small></button>
  <button data-guide-quick="travel"><strong>Travel</strong><small>Leave town, follow routes, visit another settlement.</small></button>
  <button data-guide-quick="events"><strong>See What Is Happening</strong><small>Look at festivals, local problems, and larger world events.</small></button>
  <button data-guide-quick="rest"><strong>Rest</strong><small>Recover energy and let time move forward.</small></button>
  <button data-guide-quick="eat"><strong>Eat</strong><small>Handle hunger and ordinary daily needs.</small></button>
  <button data-guide-quick="stories"><strong>Your Stories</strong><small>See which people and situations your choices are making important.</small></button>
  <button data-guide-quick="freeform"><strong>Freeform Action</strong><small>Describe something not covered by the normal buttons.</small></button>
 </div></details>
 <details class="guide-details"><summary>How does the game work?</summary><div class="guide-explainer"><p><strong>1. The world presents circumstances.</strong> People work, travel, date, argue, events unfold, and settlements change without waiting for you.</p><p><strong>2. You choose what your person cares about.</strong> There is no required main quest.</p><p><strong>3. Time moves when you act.</strong> Work, travel, training, conversations, rest, and exploration consume time.</p><p><strong>4. Your choices accumulate.</strong> NPCs remember you, relationships change, and repeated interests become personal stories.</p><p><strong>5. Freeform is optional.</strong> Use normal buttons whenever they cover what you want. Use Freeform only when you want to attempt something more specific.</p></div></details>`;
 panel.querySelector('[data-guide-main]')?.addEventListener('click',e=>guidedAction(e.currentTarget.dataset.guideMain));
 panel.querySelectorAll('[data-guide-quick]').forEach(b=>b.addEventListener('click',()=>guidanceQuickAction(b.dataset.guideQuick)));
}

function renderLife(){if(!world||!$('lifeGame'))return;const ch=activeCharacter();$('noActiveLife').classList.toggle('hidden',!!ch);$('lifeGame').classList.toggle('hidden',!ch);if(!ch)return;ensureCharacterLifeState(ch);ensureRegionalWorld(ch);const town=currentSettlement(ch),place=town.places.find(p=>p.key===ch.currentLocationKey)||town.places[0];renderHowToPlay();renderPlayScene();renderMyStory();$('lifeCharacterName').textContent=ch.name;$('lifeContext').textContent=`${ch.occupation} · ${bendingLabel(ch.bending.element)} · ${worldDate()} · ${String(world.clock.hour).padStart(2,'0')}:00 · ${world.environment.weather}`;$('townName').textContent=town.name;$('townDescription').textContent=town.description;$('currentLocationPill').textContent=place.name;$('sceneLocationName').textContent=place.name;$('sceneDescription').textContent=place.desc;
 ...
 renderDirectorGuidance(ch,town,place); const present=presentNpcs(town,ch.currentLocationKey);$('presentNpcList').innerHTML=present.length?present.map(n=>`<div class="person-row"><div><strong>${escapeHtml(n.name)}</strong><small>${escapeHtml(n.role||n.occupation||'Resident')}</small></div><button data-social="${n.id}">Talk</button></div>`).join(''):'<p class="muted">No familiar residents are here right now. Their schedules may bring them back later.</p>';$('presentNpcList').querySelectorAll('[data-social]').forEach(b=>b.addEventListener('click',()=>socialize(b.dataset.social)));
