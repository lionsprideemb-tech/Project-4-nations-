...
function newWorld(){
  ...
  world={ schemaVersion:51,id:uid(),name,era,historyMode,createdAt:new Date().toISOString(),lastSavedAt:null,clock:{year:initialYear(era),seasonIndex:0,day:1,hour:7},environment:{weather:'Clear'},characters:[],activeCharacterId:null,npcs:[
...
async function submitFreeformAction(){
  ...
  const ch=activeCharacter(),town=currentSettlement(ch); if(!ch||!town)return; const place=town.places.find(p=>p.key===ch.currentLocationKey)||town.places[0]; renderDirectorGuidance(ch,town,place); const present=presentNpcs(town,ch.currentLocationKey); const parsed=inferIntent(rawAction); const targetNpc=findMentionedNpc(rawAction,present); const feasibility=checkFeasibility(ch,town,place,present,parsed); const score=relevantSkillScore(ch,parsed.intent); const outcome=feasibility.allowed?rollOutcome(score,feasibility.difficulty):{tier:'failure',label:'Not possible as attempted',roll:null};
  const context={rawAction,ch,town,place,present,parsed,targetNpc,feasibility,score,outcome}; if(targetNpc&&parsed.intent==='talk')recordConversationMemory(ch,targetNpc,rawAction,outcome.label);witnessPlayerAction(ch,town,place,rawAction,outcome,targetNpc); btn.disabled=true;btn.textContent='Resolving…';if($('gmGenerationStatus')){$('gmGenerationStatus').textContent='Resolving the action…';$('gmGenerationStatus').classList.remove('hidden');}
  let narration=''; let provider='local';
  if(feasibility.allowed){const hours=applyFreeformConsequences(context); context.hours=hours; try{if(world.gmSettings?.mode==='connected'&&!shouldUseFastLocalNarration(context)){if($('gmGenerationStatus'))$('gmGenerationStatus').textContent='The Story Director is narrating…';narration=await connectedNarration(context);provider='connected';}else if(world.gmSettings?.mode==='connected'){provider='fast-local';}}catch(err){narration='';provider='local-fallback';world.director2=world.director2||{dailyScenes:{},latency:[],lastFallback:null};world.director2.lastFallback={worldDate:worldDate(),reason:err?.name==='AbortError'?'AI narration timed out':err?.message||String(err)};}}
  if(!narration)narration=enrichLocalNarration(context,localNarration(context));const memoryCallback=memoryCallbackFor(context);recordMemoryCallback(context,memoryCallback);detectSharedMoment(context,narration);recognizeQuestlessStory(context,narration);const directorRewardEarned=feasibility.allowed?directorReward(context):null;rememberDirectorAction(context,narration,directorRewardEarned);
  ...
  if(feasibility.allowed)advanceHours(context.hours||1);else saveWorld(); btn.disabled=false;btn.textContent='Attempt Action';if($('gmGenerationStatus'))$('gmGenerationStatus').classList.add('hidden'); renderGmPanel();
}
